...
## Adversarial fallback scope

```mermaid
flowchart TD
  Change["Changed central files"]
  Allow{"exact owner/repo:path in fallback allowlist"}
  Core["Full review + adversarial gate"]
  Fall["Reduced fallback path"]

  Change --> Allow
  Allow -->|"adversarial_evidence.py or its test"| Core
  Allow -->|"other listed core helpers"| Core
  Allow -->|"not listed"| Fall
```
...
- [`PR_GOVERNANCE_AUDIT.md`](PR_GOVERNANCE_AUDIT.md) — live review/merge
  contract.
- [`docs/doctoring/opencode-adversarial-fallback-scope.md`](docs/doctoring/opencode-adversarial-fallback-scope.md)
  — current increment's fallback-scope decision and APA 7th citations.
