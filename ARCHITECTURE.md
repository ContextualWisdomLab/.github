...
## CodeQL single-SHA gate

```mermaid
flowchart TD
  Dep["Dependabot codeql-action bump"]
  Pin{"init analyze upload-sarif same SHA?"}
  PR["codeql-pr.yml head and merge"]
  Sched["scheduled-security-scan.yml"]
  Split["Fail contract: mixed trusted action"]
...
CWE-829: a second SHA is a second control sphere.
...
- [`docs/doctoring/codeql-action-single-version.md`](docs/doctoring/codeql-action-single-version.md)
