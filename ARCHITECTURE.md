...
`orchestrator/free` gateway, but has one 15-minute process-level wall-clock deadline across open, read,
...
socket inactivity timeout, so response activity cannot renew the budget. If that corrective request encounters transport exhaustion, the typed
