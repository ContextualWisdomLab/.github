...
## Applyable suggestion vs leftover diff fence

```mermaid
flowchart TD
  Comment["Inline review comment body"]
  Fence{"Closed ```suggestion fence?"}
  Side{"side is LEFT?"}
  Apply["Overview: applyable path:line or path:start-end"]
  Left["Overview leftover: LEFT"]
  Cannot["Overview leftover: cannot-provide"]

  Comment --> Fence
  Fence -->|"yes"| Side
  Fence -->|"no, closed ```diff only"| Cannot
  Side -->|"yes"| Left
  Side -->|"no"| Apply
```

A leftover mention of the token `` ```suggestion `` is not a fence
(CWE-1288). GitHub cannot apply LEFT-side suggestions.
