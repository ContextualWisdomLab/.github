...
```mermaid
flowchart TD
  Candidate["provider/model candidate"]
  Known{"defined and reasoning-capable?"}
  Checks["reasoning=true + effort=high + variant high"]
  Ok["no errors"]
  Err["stable fail-closed messages"]

  Candidate --> Known
  Known -->|"no"| Ok
  Known -->|"yes"| Checks
  Checks -->|"all set"| Ok
  Checks -->|"any missing"| Err
```
...
- [`PR_GOVERNANCE_AUDIT.md`](PR_GOVERNANCE_AUDIT.md) — live review/merge
  contract.
- [`docs/doctoring/opencode-reasoning-effort-message-shape.md`](docs/doctoring/opencode-reasoning-effort-message-shape.md)
  — current increment's Extract Variable decision and APA 7th citations.
