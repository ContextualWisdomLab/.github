...
## Mention-dispatch envelope

```mermaid
flowchart TD
  Mention["@opencode-agent mention"]
  Envelope["Three-property versioned claim"]
  Live{"head/base/target still match?"}
  Queue["Serialize sender+PR; do not drop pending"]
  Reject["Reject stale snapshot"]
...
- [`PR_GOVERNANCE_AUDIT.md`](PR_GOVERNANCE_AUDIT.md) — live review/merge
  contract.
- [`docs/doctoring/agent-mention-dispatch-envelope.md`](docs/doctoring/agent-mention-dispatch-envelope.md)
  — current increment's envelope decision and APA 7th citations.
