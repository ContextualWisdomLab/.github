...
## 2026-09-25 - [마지막 서브스트링 탐색 시 str.find 대신 str.rfind 사용]
**Learning:** `scripts/ci/opencode_review_normalize_output.py`의 `label_section` 함수에서 특정 라벨의 마지막 위치를 찾기 위해 `str.find`를 while 루프에서 반복 호출하는 방식은 O(N)의 오버헤드를 발생시킵니다.
**Action:** 긴 문자열에서 특정 서브스트링의 마지막 인스턴스를 찾을 때, `str.find`로 앞부터 순차 스캔하여 결과를 리스트에 모으지 말고, `str.rfind()`를 사용해 뒤에서부터 직접 검색하여 불필요한 반복 O(N) 스캐닝을 방지하십시오.
