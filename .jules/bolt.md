...
## 2026-08-22 - 스냅샷 수집 병목 현상 개선
**Learning:** `organization_commercial_readiness_loop.py` 내부의 리포지토리 스냅샷 수집 로직(`client.snapshot`)을 동기식 반복문으로 실행하면 저장소 수에 비례하여 N+1 API 병목 현상이 발생하고 파이프라인의 실행 속도를 심각하게 지연시킵니다.
**Action:** `concurrent.futures.ThreadPoolExecutor`를 사용하여 외부 서비스 호출 시 네트워크 병목을 해소하되, API 속도 제한을 고려하여 적절한 `max_workers` 값으로 제한하여 사용합니다.
