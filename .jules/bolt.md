...
## 2026-11-20 - Replace regex finditer with native str.find for loop scanning
**Learning:** Found a codebase-specific anti-pattern in `scripts/ci/opencode_review_normalize_output.py` where deep label-scanning over long review texts (LLM output) was using `pattern.finditer()` inside loops. Micro-benchmarking shows that native `str.find()` inside a `while index != -1:` loop is substantially (~40%) faster for exact string matching without the regex execution overhead.
**Action:** When scanning large strings for exact literal substrings (like known label prefixes) inside nested search loops, avoid `re.compile(re.escape(string)).finditer()` and use Python's heavily optimized native `text.find(candidate, index)` method instead, ensuring the index advances on all branches to avoid infinite loops.
## 2026-11-20 - [대용량 텍스트 스캔 시 정규표현식 대신 네이티브 메서드 활용]
