...
## 2026-09-22 - 반복문 내 정규식 추출 및 C 속도 문자열 메서드 전환을 통한 최적화
**Learning:** `scripts/ci/opencode_review_normalize_output.py`에서 텍스트 정규화에 사용되던 정규표현식(`re.sub(r"\s+", "-", ...)`)과 스캐닝 루프 내 인라인 `re.split`이 캐시 룩업 및 컴파일 오버헤드를 유발하여 성능 병목이 되고 있음을 발견했습니다.
**Action:** 내부 탐색 루프에서 반복되는 정규표현식은 모듈 레벨에서 컴파일된 상태로 선언하여 재사용하고, 단순 공백 문자열 정규화 등의 작업은 정규식 대신 고도로 최적화된 C 속도의 파이썬 네이티브 문자열 메서드(`str.split()`과 `str.join()`)를 결합하여 처리하도록 최적화하십시오.
