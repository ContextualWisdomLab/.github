...
## 2026-09-12 - [정규표현식 공백 치환(re.sub)을 네이티브 문자열 split/join으로 최적화]
**Learning:** `scripts/ci/opencode_review_normalize_output.py`의 `runtime_tool_slug`에서 임의의 공백 문자열을 단일 하이픈(`-`)으로 치환할 때 `re.sub(r"\s+", "-", ...)`를 사용하는 것은 정규표현식 컴파일 및 매칭 오버헤드로 인해 비효율적입니다. 마이크로 벤치마크 결과, 파이썬 네이티브 문자열 메서드인 `"-".join(text.split())`을 활용하는 것이 약 4~6배 더 빠릅니다. 정규표현식 엔진을 거치지 않고 C 레벨에서 네이티브하게 동작하기 때문입니다.
**Action:** 단순한 공백 정규화나 문자열 치환의 경우, 정규표현식(`re.sub`)보다 `str.split()`과 `str.join()`을 조합하는 네이티브 방식을 우선적으로 사용하십시오.
