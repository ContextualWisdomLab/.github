...
## 2026-08-25 - 문자열 리터럴 검색을 위한 불필요한 정규표현식 컴파일 제거
**Learning:** `opencode_review_normalize_output.py`의 `label_starts` 함수 내부에서 레이블 텍스트 매칭 시 `re.escape(candidate)`와 정규식 `finditer`를 사용하고 있었으나, 이는 고정 문자열에 대해 과도한 리소스를 소모하는 안티 패턴입니다.
**Action:** 고정된 리터럴 문자열을 검색할 때는 정규식 컴파일(`re.compile`)을 사용하는 대신 내장 `str.find(candidate, index)`를 while 루프 안에서 호출하여, O(N)의 빠른 단순 문자열 검색으로 성능을 최적화하십시오.
