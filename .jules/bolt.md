...
## 2024-05-14 - Optimize regex operations in bash wrapper script

**Learning:** 파이썬에서 CI 로그와 같은 대량의 문자열에 대해 여러 번의 정규식 치환을 순차적으로 적용할 경우, 패턴마다 중복 스캔이 발생하여 불필요한 성능 저하(O(N) 오버헤드)가 발생합니다. 파이썬 `re` 모듈은 매 라인마다 치환을 수행하므로 이 비용이 커집니다.
**Action:** 동일한 텍스트에 여러 정규식 치환을 적용해야 할 때는 항상 `|` (OR) 연산자를 사용해 하나의 컴파일된 정규식 패턴으로 통합하여, 문자열 스캔을 1회로 줄여 성능을 최적화해야 합니다.
