...
## 2024-05-19 - [대용량 문자열 서브스트링 스캐닝 시 뒤에서부터 찾기 최적화]
**Learning:** `scripts/ci/opencode_review_normalize_output.py`의 `label_section` 함수에서 특정 라벨의 가장 마지막 등장 위치를 찾기 위해 `text.find()`를 반복문으로 여러 번 호출하고 결과를 리스트에 저장하는 패턴이 있었습니다. 이는 긴 텍스트에서 불필요한 전체 문자열 스캔 및 O(N) 반복을 유발하여 병목이 됩니다.
**Action:** 마지막 등장 위치만 필요한 경우, `text.find()`를 이용한 순차 스캔 대신 `text.rfind()`를 사용하여 역방향으로 한 번에(또는 필요한 만큼만) 검색하십시오. 이를 통해 불필요한 스캐닝 오버헤드를 막아 엄청난 성능 향상을 이끌어낼 수 있습니다.
