...
## 2026-09-09 - [대용량 텍스트 정규표현식 화이트스페이스 정규화 최적화]
**Learning:** `scripts/ci/opencode_review_normalize_output.py`의 `runtime_tool_slug`와 같이 자주 호출되는 텍스트 변환 함수 내부에서 `re.sub(r"\s+", "-", ...)` 패턴을 매번 인라인으로 파싱하고 평가하는 것은 불필요한 캐시 룩업 및 정규표현식 실행 오버헤드를 발생시킵니다.
**Action:** 화이트스페이스를 단일 문자로 치환하는 단순한 정규화 작업의 경우 정규표현식을 사용하지 않고 고도로 최적화된 C 속도의 네이티브 문자열 메서드 조합인 `"-".join(text.split())`을 활용하여 오버헤드를 제거하십시오.
