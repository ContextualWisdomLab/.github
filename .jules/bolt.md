...
## 2026-09-09 - [Fast Path Regex Mentions]
**Learning:** 복잡한 lookaround를 포함한 정규식 엔진을 임의의 사용자 텍스트(예: PR 댓글)에 적용하면 성능이 크게 저하될 수 있습니다.
**Action:** 임의의 텍스트에 정규식을 적용하기 전에 필요한 특정 문자(예: `@` 또는 `/`)의 존재 여부를 미리 확인하는 빠른 문자열 검사(substring check)를 항상 고려해야 합니다.

## 2026-09-09 - [Newlines in command substitution output]
**Learning:** `grep -vx "string" file | sed -n '1,20p'`와 같이 커맨드 치환(`$()`)을 사용하여 파일에서 추출한 여러 줄의 결과를 bash 변수에 할당할 때, bash는 변수 내부의 개행 문자를 보존하지만 `if [ -n "$sidecar_startup_warnings" ]` 안에서 `log` 함수 등으로 전달할 때 단일 문자열로 처리됩니다. 원래 에러 발생 시 `grep`이 매칭에 실패하여 `sed`가 빈 문자열을 반환해야 하지만, `grep -v`는 찾지 못한 모든 줄을 반환하므로 전체 로그가 변수에 할당되고, 이것이 테스트에서 여러 줄로 나오면서 단일 문자열 매칭에 실패하게 만들 수 있습니다. 특히 `tr '\n' ' '`을 추가하면 개행 문자가 스페이스로 변경되어, 에러 메시지나 로그 검사 시 개행으로 인한 잘림/매칭 실패 문제를 방지할 수 있습니다.
**Action:** 여러 줄을 반환할 수 있는 shell 명령어의 출력을 변수에 담아 다른 명령어(특히 `echo`나 `log`)의 인자로 사용할 때는 `tr '\n' ' '` 파이프라인을 추가하여 출력을 단일 줄로 평탄화(flatten)하는 것을 항상 고려하세요.
