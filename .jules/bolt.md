...
## 2026-08-09 - [Avoid character-by-character append in Python parsing loops]
**Learning:** `strip_jsonc_comments` in `scripts/ci/assert_opencode_reasoning_effort.py` was appending characters one-by-one inside a `while` loop, which leads to slow execution on large input strings due to python loop overhead and list appending.
**Action:** In Python parsing logic, avoid appending single characters in tight linear loops. Instead, pre-compile regex at the module level using `re.compile()` and use `re.sub` with a custom replacer to batch unmatched text, or scan ahead and use string slices (e.g., `output.append(text[start:cursor])`).
