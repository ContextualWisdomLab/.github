...
**Learning:** `scripts/ci/opencode_review_normalize_output.py`의 `runtime_tool_slug` 함수와 같이 단일 문자열에서 연속된 공백 문자를 단일 문자(예: 대시 `-`)로 치환할 때, `re.sub(r"\s+", "-", ...)`와 같은 정규표현식을 사용하면 상당한 정규표현식 파서 컴파일 및 실행 오버헤드가 발생합니다. 이는 대량의 텍스트나 로그를 분석하는 CI 스크립트에서 병목을 일으킬 수 있습니다.
**Action:** 단순한 공백 문자의 정규화(치환) 작업에는 오버헤드가 큰 정규표현식 대신 파이썬의 고도로 최적화된 C-speed 네이티브 문자열 메서드인 `"-".join(text.split())`을 활용하십시오. 이는 마이크로 벤치마크 결과로 입증된 훨씬 빠른 성능을 보장합니다.
