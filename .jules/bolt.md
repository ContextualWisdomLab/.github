...
## 2026-09-17 - [문자열 치환 시 re.sub 대신 네이티브 메서드 활용 및 루프 내 re.split 컴파일 오버헤드 제거]
**Learning:** Python에서 단순한 공백 제거 후 하이픈으로 연결하는 등의 문자열 치환 작업 시, `re.sub`를 사용하는 것보다 네이티브 문자열 메서드인 `split()`과 `join()`을 조합하는 것이 훨씬(수 배 이상) 빠릅니다. 또한 자주 호출되는 함수나 루프 내부에서 인라인 정규표현식 리터럴과 함께 `re.split`을 사용하면 불필요한 컴파일 오버헤드가 발생합니다.
**Action:** 단순한 문자열 치환에는 네이티브 문자열 메서드(예: `"-".join(text.split())`)를 우선적으로 고려하고, 반복적으로 사용되는 복잡한 패턴의 분리 작업 시에는 반드시 `re.compile`을 사용해 패턴을 모듈 레벨 변수로 추출한 후 `pattern.split()`을 호출하십시오.
