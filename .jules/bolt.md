...
## 2024-11-23 - 파이썬 문자열 조작: 정규표현식 치환 대신 네이티브 분할 및 연결 메서드 활용
**Learning:** Python에서 연속된 공백을 하이픈 등으로 치환하는 간단한 정규화 작업 시, `re.sub(r"\s+", "-", string)`을 사용하는 것은 네이티브 문자열 메서드인 `"-".join(string.split())`을 사용하는 것보다 현저히 느립니다. 마이크로 벤치마크 결과 정규표현식을 미리 컴파일하더라도 네이티브 메서드가 대략 3~4배 더 빠릅니다. 자주 호출되거나 반복문 내부에 위치한 코드 경로에서 정규표현식 엔진의 초기화 및 탐색 오버헤드는 불필요하게 성능을 갉아먹습니다.
**Action:** 단순하고 예측 가능한 형태의 공백 정규화 작업 시에는 항상 정규표현식 치환(`re.sub`)보다 네이티브 C 속도로 동작하는 문자열 메서드(`str.split()` 및 `str.join()`)를 우선적으로 사용하십시오.
