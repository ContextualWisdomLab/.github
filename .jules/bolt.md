...
## 2026-09-21 - [대용량 텍스트 스캔 시 정규표현식 대신 네이티브 메서드 활용 최적화]
**Learning:** 단순 공백 제거 및 문자열 정규화 작업에서 `re.sub(r"\s+", "-", ...)`를 사용하는 것은 파이썬 내장 메서드인 `"-".join(string.split())`에 비해 불필요한 정규표현식 엔진 호출 오버헤드를 유발하며 성능이 떨어집니다.
**Action:** 단순 공백 치환이나 텍스트 정규화 작업에서는 항상 `re.sub` 대신 네이티브 C-speed 문자열 메서드(`split()`, `join()`)를 선호하여 마이크로 최적화를 수행하십시오.
