...
## 2026-08-22 - Optimize JSON Extraction with JSONDecoder().raw_decode()
**Learning:** Found an opportunity to replace `rfind` and string slicing in `scripts/ci/noema_review_gate.py`'s `extract_json_object`. Using `json.JSONDecoder().raw_decode()` safely avoids O(N) memory allocations for substrings while perfectly preventing bugs caused by trailing garbage characters.
**Action:** When extracting JSON from a string that might contain trailing non-JSON text (like LLM output), prefer `json.JSONDecoder().raw_decode()` over `rfind("}")` to make parsing faster and more robust.
