...
## 2024-11-28 - [성능 개선] 다중 라벨 텍스트 스캔 정규표현식 최적화
**Learning:** `scripts/ci/opencode_review_normalize_output.py`의 `label_section` 함수는 다음 섹션의 시작 위치를 찾기 위해 *모든* 검증 라벨에 대해 전체 남은 텍스트를 재스캔하는 리스트 컴프리헨션을 사용하여 `O(L * N)` 복잡도를 가졌으며, 이는 패턴이 많은 대용량 텍스트 로그에서 심각한 성능 병목이었습니다.
**Action:** 다중 하위 문자열 패턴에 대한 순차적 검색을 단일 컴파일된 정규표현식(`ANY_LABEL_PATTERN = re.compile("|".join(...))`)으로 결합하고 `.search(text, start)`를 사용하여 다음 매칭 섹션 마커로 O(N) 시간 내에 이동하도록 변경했습니다.
