...
## 2024-10-24 - Avoid N+1 API blocking in organization commercial readiness loop
**Learning:** In backend processing scripts, synchronous iterations calling an external service, such as fetching client.snapshot per repository in organization_commercial_readiness_loop.py, cause N+1 API bottlenecks and stall pipeline execution linearly.
**Action:** Use concurrent.futures.ThreadPoolExecutor for independent network calls in a loop when there are multiple items, keep empty and single-item inputs on the cheaper serial path, and bound max_workers to avoid API rate limits.
