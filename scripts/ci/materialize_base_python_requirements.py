...
TRUSTED_UV_RETRY_BACKOFF_SECONDS = 1.0
TRUSTED_UV_RETRYABLE_HTTP_CODES = frozenset({408, 429, 500, 502, 503, 504})
...
def _download_trusted_uv_archive() -> bytes:
    ...
    while True:
        ...
        try:
            ...
            if (
                exc.code not in TRUSTED_UV_RETRYABLE_HTTP_CODES
                or attempt >= TRUSTED_UV_DOWNLOAD_ATTEMPTS
            ):
                raise RuntimeError(
                    f"trusted uv archive download failed: HTTPError {exc.code}"
                ) from exc
        except OSError as exc:
            if attempt >= TRUSTED_UV_DOWNLOAD_ATTEMPTS:
                raise RuntimeError(
                    f"trusted uv archive download failed: {type(exc).__name__}"
                ) from exc
        time.sleep(TRUSTED_UV_RETRY_BACKOFF_SECONDS)
