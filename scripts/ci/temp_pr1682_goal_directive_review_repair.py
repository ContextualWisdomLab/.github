#!/usr/bin/env python3
"""Repair PR #1682 review findings and normalize generated Markdown EOF."""
...
"""
DOCTORING_APPEND = """## 2026-09-02 directive sync and review reconciliation
...
def main() -> int:
    """Apply reviewed corrections, enforce one-LF EOFs, then self-delete."""
    ...
    DIRECTIVE.write_text(directive.rstrip() + "\n", encoding="utf-8")

    doctoring = DOCTORING.read_text(encoding="utf-8")
    doctoring = replace_exact(doctoring, OLD_DOCTORING_ROUTE, NEW_DOCTORING_ROUTE, "stale doctoring route")
    marker = "## 2026-09-02 directive sync and review reconciliation"
    if marker not in doctoring:
        doctoring = doctoring.rstrip() + "\n\n" + DOCTORING_APPEND
    DOCTORING.write_text(doctoring.rstrip() + "\n", encoding="utf-8")
