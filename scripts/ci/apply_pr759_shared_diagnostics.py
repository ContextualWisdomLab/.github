#!/usr/bin/env python3
"""Apply the exact reviewed PR 759 diagnostics repair, then remove this helper."""
...
def replace_once(path: str, old: str, new: str) -> None:
    """Replace one exact source fragment and fail when the branch has drifted."""
    file_path = Path(path)
...
def remove_top_level_function(path: str, function_name: str) -> None:
    """Remove one top-level Python function and its trailing blank lines."""
    file_path = Path(path)
    text = file_path.read_text(encoding="utf-8")
...
def align_codeql_pins(path: str) -> None:
    """Align CodeQL init and analyze to the reviewed immutable v4.37.5 commit."""
    file_path = Path(path)
    text = file_path.read_text(encoding="utf-8")
    target = "d1ba80a13dd99fba24a470575428917156a28b43 # v4.37.5"
    for old in (
    ...
    if text.count(target) != 2:
        raise SystemExit(f"{path}: CodeQL init/analyze pins are not exactly aligned")
    file_path.write_text(text, encoding="utf-8")


def main() -> int:
    """Apply production, test, standards, and cleanup changes atomically."""
    shared_import = '''sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[2]))
...
'''
    ...
    remove_top_level_function(javascript_path, "_publish_coverage_failure_summary")
    ...
    remove_top_level_function(python_path, "_publish_coverage_failure_summary")

    helper_path = "scripts/ci/coverage_failure_summary.py"
    replace_once(
        helper_path,
        '''import html
import os
...
''',
    ...
    replace_once(
        helper_path,
        "redacted = _SANITIZE_TEXT(normalized)",
        "redacted = sanitize_text(normalized)",
    )

    sanitizer_path = "scripts/ci/sanitize_github_output_summary.py"
    replace_once(
        sanitizer_path,
...
''',
    ...
    sanitizer_test = "tests/test_sanitize_github_output_summary.py"
    replace_once(sanitizer_test, "import runpy\nimport sys\n", "import sys\n")
    replace_once(
        sanitizer_test,
        "from scripts.ci.sanitize_github_output_summary import sanitize_text",
...
''',
    ...
    assertion = '\tassert_file_contains "$GATE_SCRIPT" "Dockerfile | */Dockerfile | Dockerfile.* | */Dockerfile.* | Containerfile | */Containerfile | Makefile | */Makefile" "strix gate treats deployment files as source files"\n'
    replace_once(
        strix_test,
        assertion,
        assertion
    ...
    align_codeql_pins(".github/workflows/codeql-pr.yml")
    align_codeql_pins(".github/workflows/scheduled-security-scan.yml")

    Path("docs/doctoring/coverage-failure-diagnostics.md").write_text(
        '''# Credential-redacted coverage failure diagnostics

## Decision

Coverage setup failures are review evidence, but exception text is untrusted and may contain registry URL userinfo, authorization headers, API tokens, database connection strings, passwords, or encryption keys. JavaScript and Python trusted-lock materializers therefore delegate multiline `GITHUB_OUTPUT` publication to one shared helper. The helper normalizes whitespace, applies the central credential sanitizer, bounds each field, HTML-escapes Markdown evidence, and replaces the fixed multiline delimiter before publication.
...
The exact-head gate requires Python 3.10 compilation, Python 3.14 tests, 100% production statement and branch coverage, 100% production docstrings, and direct execution of the sanitizer entrypoint contract. Regression cases cover mixed URL, bearer, and token secrets; delimiter injection; oversized errors; missing `GITHUB_OUTPUT`; and both materializer paths. Temporary write-capable repair automation is absent from the final tree.
...
''',
    ...
    Path(".github/workflows/repair-pr759-shared-diagnostics.yml").unlink(missing_ok=True)
    Path(".github/workflows/canonicalize-opencode-diagnostics.yml").unlink()
