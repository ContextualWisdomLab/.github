...
# the org's own contextual-orchestrator repo at a pinned ref, installs its
# hash-locked dependencies, registers whichever of the five upstream provider
# credentials are present as job secrets into its KV, runs model auto-discovery
# + cost-based auto-enable, then serves on 127.0.0.1 with a freshly generated
# bearer token. Intended to be sourced/invoked as an early step in a job that
# later calls into OpenCode/Noema/Strix so they can use contextual-orchestrator
# as their LLM backend. Not for public/shared exposure: loopback-only, one
# ephemeral token per job run, torn down with the runner.
set -euo pipefail

: "${GITHUB_ENV:=/dev/null}"
: "${GITHUB_OUTPUT:=/dev/null}"
: "${CONTEXTUAL_ORCHESTRATOR_REF:=main}"
...
if [[ ! "$CONTEXTUAL_ORCHESTRATOR_REF" =~ ^[0-9a-fA-F]{40}$|^[A-Za-z0-9._/-]+$ ]]; then
	echo "ERROR: CONTEXTUAL_ORCHESTRATOR_REF must be a git SHA or a plain ref name." >&2
	exit 2
fi
...
until curl -fsS "http://127.0.0.1:${CONTEXTUAL_ORCHESTRATOR_PORT}/healthz" >/dev/null 2>&1; do
