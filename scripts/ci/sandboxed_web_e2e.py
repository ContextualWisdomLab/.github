...
def tail_text(
    ...
    """Return redacted final lines from one byte-bounded service evidence file."""
    if not path.exists():
        return ""
    if max_lines <= 0:
...
def main(argv: Sequence[str] | None = None) -> int:
    """Run backend, frontend, and E2E commands inside a bounded sandbox copy."""
    ...
    try:
        try:
            ...
            if args.allow_env:
                print(
                    "sandboxed-web-e2e: allowed env names="
                    f"{','.join(sorted(set(args.allow_env)))}"
                )
            ...
            if _services_output_limited(services):
                ...
                print(
                    "sandboxed-web-e2e: service readiness failed",
                    file=sys.stderr,
                )
                exit_code = 125
            else:
                try:
                    ...
                    output_limited = bool(
                        getattr(exc, "output_limited", False)
                    )
                    print(
                        "sandboxed-web-e2e: e2e command timed out after "
                        f"{args.e2e_timeout}s",
                        file=sys.stderr,
                    )
                    exit_code = 124
