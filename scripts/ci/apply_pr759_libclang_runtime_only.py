...
def _require_permanent_diagnostics() -> None:
    """Fail unless the reviewed permanent diagnostics workflow is present."""
    workflow = DIAGNOSTICS_PATH.read_text(encoding="utf-8")
    contract = '      - "tests/test_opencode_libclang_toolchain_contract.py"'
    if workflow.count(contract) != 2:
        raise SystemExit("permanent diagnostics workflow lost the libclang contract")
...
def main() -> None:
    ...
    _require_permanent_diagnostics()
