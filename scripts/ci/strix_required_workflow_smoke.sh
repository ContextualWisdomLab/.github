...
assert_file_contains "$workflow_file" "validate_strix_runtime_compatibility.py" "Strix workflow verifies exact pins and executable cryptographic consumers"
...
assert_file_not_contains "$workflow_file" "Treating as a neutral skip" "Strix required wrapper never converts incomplete scans to success"
assert_file_contains "$gate_script" "import-current-attempt" "Strix gate imports authoritative current-attempt reports before judgment"
assert_file_contains "$gate_script" "vulnerability_file_is_self_negating_no_finding" "Strix gate recognizes contradictory no-finding pseudo-records"
