...
assert_file_not_contains "$workflow_file" "Materialize central Strix dependency lock from PR head" "Strix workflow must never execute dependency metadata from PR head"
assert_file_not_contains "$workflow_file" 'PR_HEAD_SHA:requirements-strix-ci-hashes.txt' "Strix workflow must never read the dependency lock from PR head"
assert_file_contains "$workflow_file" 'trusted_lock="$TRUSTED_STRIX_SOURCE/requirements-strix-ci-hashes.txt"' "Strix workflow selects the lock from the trusted central checkout"
