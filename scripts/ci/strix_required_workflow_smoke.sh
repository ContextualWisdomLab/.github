...
assert_file_contains "$workflow_file" "github.event.pull_request.user.login == 'dependabot[bot]'" "Strix PR-head lock validation is limited to Dependabot"
assert_file_contains "$workflow_file" "startsWith(github.event.pull_request.head.ref, 'dependabot/pip/')" "Strix PR-head lock validation is limited to Dependabot pip branches"
assert_file_contains "$workflow_file" '"${#changed_files[@]}" -ne 1' "Strix Dependabot validation requires exactly one changed file"
assert_file_contains "$workflow_file" '"$lock_mode" != "100644"' "Strix Dependabot validation requires a regular Git blob"
