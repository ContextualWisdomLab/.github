...
assert_file_contains "$gate_script" "STRIX_OPENAI_API_KEY_FILE" "Strix gate supports direct-OpenAI fallback credentials for cross-provider fallback"
assert_file_contains "$gate_script" "STRIX_OPENAI_API_BASE_FILE" "Strix gate supports a dedicated direct-OpenAI fallback endpoint"
...
assert_file_not_contains "$workflow_file" "openai-direct/gpt-5.6-luna" "Strix fallback list must use the underscore openai_direct/ form litellm actually routes on"
