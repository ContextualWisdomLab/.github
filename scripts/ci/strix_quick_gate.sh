...
for pattern in patterns:
    for match in pattern.finditer(text):
        ...
        end = (match.groupdict().get('end') or start).strip()
        key = (value, start, end)
        if value and key not in seen:
            seen.add(key)
for value, start, end in sorted(seen):
...
PY
	...
	{
		local start_line end_line normalized_location
		while IFS=$'\t' read -r location start_line end_line; do
			normalized_location="$(normalize_vulnerability_location "$location")" || continue
			printf '%s\t%s\t%s\n' "$normalized_location" "$start_line" "$end_line"
...
extract_vulnerability_locations() {
	local vuln_file="$1"
	local location _start_line _end_line
	while IFS=$'\t' read -r location _start_line _end_line; do
		printf '%s\n' "$location"
	done < <(extract_vulnerability_location_records "$vuln_file") | sort -u
}
...
evaluate_pull_request_findings() {
	...
	for run_dir in "$STRIX_REPORTS_DIR"/*; do
		...
		for vuln_file in "$vulnerabilities_dir"/*.md "$vulnerabilities_dir"/*.json; do
			...
			found_baseline_threshold_finding=1
			local changed_file vulnerability_record vulnerability_location vulnerability_start_line vulnerability_end_line
			for vulnerability_record in "${vulnerability_location_records[@]}"; do
				IFS=$'\t' read -r vulnerability_location vulnerability_start_line vulnerability_end_line <<<"$vulnerability_record"
				for changed_file in "${CHANGED_FILES[@]}"; do
	...
	if [ "$found_baseline_threshold_finding" -eq 0 ] && [ "$found_changed_manifest_only_threshold_finding" -eq 0 ]; then
		...
		if [ "$rank" -ge "$threshold_rank" ]; then
			mapfile -t vulnerability_location_records < <(extract_vulnerability_location_records "$STRIX_LOG")
...
has_detected_infrastructure_error() {
	local provider_failure_word_regex='(^|[^[:alpha:]])(Fatal|Denied|Warn|Warning)([^[:alpha:]]|$)'
	local provider_failure_context_regex="($LLM_PROVIDER_ONLY_REGEX|HF Hub)"
	if LC_ALL=C grep -Eiv '^[[:space:]]*(│[[:space:]]*)?MODEL QUALITY WARNING([[:space:]]*│)?[[:space:]]*$|^Warning: You are sending unauthenticated requests to the HF Hub\. Please set a HF_TOKEN to enable higher rate limits and faster downloads\.$|^[0-9]{4}-[0-9]{2}-[0-9]{2} [0-9:.]+ WARNING [^ ]+ - strix\.tools\.web_search\.tool: web_search invoked without PERPLEXITY_API_KEY configured$' "$STRIX_LOG" |
		grep -Eiq "($provider_failure_context_regex.*$provider_failure_word_regex|$provider_failure_word_regex.*$provider_failure_context_regex)"; then
		return 0
	fi
