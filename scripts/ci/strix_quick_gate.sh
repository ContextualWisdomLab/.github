...
has_unexpected_strix_console_failure_signal() {
	...
	while IFS= read -r signal_line; do
		...
		# not on its recommended frontier list. It is a quality advisory, not
		# evidence that provider execution or the scan was incomplete.
		if printf '%s\n' "$signal_line" |
			grep -Eq '^[^[:alnum:]]*MODEL QUALITY WARNING[^[:alnum:]]*$'; then
			continue
		fi
		return 0
...
has_detected_infrastructure_error() {
	if has_unexpected_strix_console_failure_signal; then
		return 0
	fi
