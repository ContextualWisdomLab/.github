...
export_and_validate_session_output() {
	...
	local export_timeout_seconds="${OPENCODE_EXPORT_TIMEOUT_SECONDS:-180}"
...
run_one_model_attempt() {
	...
	if [ -z "$session_id" ] || [ "$session_id" = "null" ]; then
		...
		if session_id="$(lookup_session_id_by_exact_title "$session_title" "$session_list_file")"; then
			printf 'OpenCode %s attempt %s/%s recovered the uniquely titled current-run session.\n' \
				"$model_candidate" "$attempt" "$attempts"
		else
			session_id=""
		fi
	...
	if [ "$opencode_status" -ne 0 ]; then
		...
		if { [ "$opencode_status" -eq 124 ] || [ "$opencode_status" -eq 137 ] || [ "$opencode_status" -eq 143 ]; } && \
			...
			printf 'OpenCode %s attempt %s/%s timed out after %ss; attempting to recover any assistant text already persisted in the uniquely titled current-run session before falling through.\n' \
				"$model_candidate" "$attempt" "$attempts" "$run_timeout_seconds"
...
main() {
	...
	budget_seconds="${OPENCODE_TOTAL_RETRY_BUDGET_SECONDS:-11700}"
	...
	if [ "${CENTRAL_REVIEW_PROCESS_FALLBACK_ELIGIBLE:-false}" = "true" ]; then
		...
		if changed_file_count="$(count_changed_files_for_cadence)"; then
			if [ "$changed_file_count" -le "$small_file_threshold" ]; then
				original_run_timeout="$(env_integer_or_default OPENCODE_SMALL_CHANGE_RUN_TIMEOUT_SECONDS 5400)"
				budget_seconds="$(env_integer_or_default OPENCODE_SMALL_CHANGE_TOTAL_BUDGET_SECONDS 11700)"
			elif [ "$changed_file_count" -le "$medium_file_threshold" ]; then
				original_run_timeout="$(env_integer_or_default OPENCODE_MEDIUM_CHANGE_RUN_TIMEOUT_SECONDS 5400)"
				budget_seconds="$(env_integer_or_default OPENCODE_MEDIUM_CHANGE_TOTAL_BUDGET_SECONDS 11700)"
			else
