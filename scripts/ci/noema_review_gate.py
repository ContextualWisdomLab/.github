...
def call_llm(
    ...
    second, potentially multi-hour ``NOEMA_LLM_REQUEST_TIMEOUT_SECONDS`` call
    on a review that ``inspect_and_review``'s own post-call stale-head check
    would discard anyway once this function returns. See ``fetch_pr`` for the
    live lookup and ``StaleHeadDuringRepairRetryError`` for how that stale
