...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 30' "opencode approval publication releases queue capacity after a short bounded gate window"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 30' "opencode approval step has a bounded wall-clock timeout that prevents duplicate model runs from holding the queue"
	assert_file_contains "$workflow_file" 'focused failed-check diagnosis a bounded single-model window' "opencode failed-check diagnosis finishes inside the bounded publication step"
