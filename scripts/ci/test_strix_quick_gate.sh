...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "github.event.pull_request.head.repo.full_name == github.event.pull_request.base.repo.full_name" "opencode pull_request coverage execution is limited to same-repository PR heads using the target PR base repo"
	...
	assert_file_contains "$workflow_file" "Await current-head R CMD check package evidence" "opencode R package evidence waits for the package-aware current-head check"
	assert_file_contains "$workflow_file" 'deadline=$((SECONDS + 5400))' "opencode R package evidence allows long repository checks to run for 90 minutes"
	assert_file_contains "$workflow_file" 'select(.head_sha == $head_sha)' "opencode R package evidence is bound to the current PR head"
	assert_file_contains "$workflow_file" 'select(.path as $path | $workflow_paths | index($path))' "opencode R package evidence accepts only the repository workflow path that declares the pinned check action"
	assert_file_contains "$workflow_file" "current-head R CMD check completed successfully" "opencode R coverage reports the authoritative package-aware peer result"
	...
	assert_file_contains "$workflow_file" "testthat unavailable in coverage runner; no package-aware peer evidence exists." "opencode R package tests fail closed when no peer package check exists"
	...
	assert_file_contains "$workflow_file" "pr_head_ref:" "opencode repository_dispatch carries the PR head branch required by current-head code-scanning verification"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 205' "opencode model pool gives two long-review candidates a bounded review window"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "5400"' "opencode primary review preserves repositories that require more than one hour"
	assert_file_contains "$workflow_file" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "11700"' "opencode model pool leaves enough budget for a second long-review candidate"
	assert_file_contains "$workflow_file" 'OPENCODE_SMALL_CHANGE_RUN_TIMEOUT_SECONDS: "5400"' "opencode does not infer repository review cost from a small changed-file count"
	assert_file_contains "$workflow_file" 'OPENCODE_MEDIUM_CHANGE_RUN_TIMEOUT_SECONDS: "5400"' "opencode medium changes preserve full-hour repository reviews"
	assert_file_contains "$workflow_file" 'OPENCODE_LARGE_CHANGE_RUN_TIMEOUT_SECONDS: "5400"' "opencode large changes preserve full-hour repository reviews"
	assert_file_contains "$workflow_file" 'OPENCODE_UNKNOWN_CHANGE_RUN_TIMEOUT_SECONDS: "5400"' "opencode unknown changes fail safe with a long-review window"
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS:-11700' "opencode model pool keeps a bounded 195-minute retry budget unless the workflow explicitly disables it"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "5400"' "opencode catalog allows a 90-minute long-repository review"
	...
	assert_file_contains "$workflow_file" 'event_type: "opencode-review"' "opencode deferred review retry uses the dedicated repository-dispatch event"
	assert_file_contains "$workflow_file" 'repos/${CENTRAL_WORKFLOW_REPOSITORY}/dispatches' "opencode deferred review retry targets the central default-branch dispatch endpoint"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 205' "opencode model stage has a bounded multi-provider timeout"
