...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 120' "opencode approval publication releases queue capacity after a bounded gate window"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 120' "opencode approval step has a bounded wall-clock timeout that prevents duplicate model runs from holding the queue"
	assert_file_contains "$workflow_file" 'focused failed-check diagnosis the same 90-minute deep-review room' "opencode failed-check diagnosis finishes inside the bounded publication step"
