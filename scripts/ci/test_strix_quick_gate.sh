...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$GATE_SCRIPT" "trap 'rm -f -- \"\$diff_output_file\"' RETURN" "strix gate always cleans temporary diff files on function return"
	assert_file_contains "$GATE_SCRIPT" 'if python3 - "$diff_output_file" "$start_line" "$end_line"' "strix gate captures changed-line overlap exit code without set -e aborting cleanup"
