...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "steps.gate.outputs.provider_mode == 'nvidia_nim' && 'nvidia_nim/nvidia/nemotron-4-340b-instruct openai-direct/gpt-5.4'" "strix workflow gives NVIDIA NIM scans contracted fallbacks"
...
assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" 'OPENCODE_NVIDIA_NIM_TOTAL_BUDGET_SECONDS 900' "opencode NVIDIA NIM combined runtime cap defaults to fifteen minutes"
	...
	assert_file_contains "$workflow_file" "nvidia-nim/nvidia/nemotron-4-340b-instruct nvidia-nim/nvidia/llama-3.1-nemotron-ultra-253b-v1 nvidia-nim/nvidia/nemotron-3-super-120b-a12b nvidia-nim/nvidia/nemotron-3-ultra-550b-a55b nvidia-nim/meta/llama-3.3-70b-instruct nvidia-nim/deepseek-ai/deepseek-v4-pro nvidia-nim/mistralai/codestral-22b-instruct-v0.1 opencode-free/nemotron-3-ultra-free" "opencode review keeps all NVIDIA NIM candidates inside the public-repository pool"
	...
	assert_file_contains "$opencode_config" '"model": "nvidia-nim/nvidia/nemotron-4-340b-instruct"' "opencode config defaults review sessions to NVIDIA NIM Nemotron Super"
