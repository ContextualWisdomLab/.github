...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "needs: [changed-scope, admit-current-head, cancel-superseded-pr-runs]" "strix provider queue waits for live-head admission and superseded-run cleanup"
	assert_file_contains "$workflow_file" "needs.cancel-superseded-pr-runs.result == 'success'" "strix provider accepts successful cleanup before execution"
	assert_file_contains "$workflow_file" "needs.cancel-superseded-pr-runs.result == 'skipped'" "strix provider preserves non-cleanup event admission"
	...
	assert_file_contains "$workflow_file" '.event == "pull_request_target" or .event == "repository_dispatch"' "strix cleanup includes native and dispatched PR scans"
	assert_file_contains "$workflow_file" 'RUN_REPOSITORY: ${{ github.repository }}' "strix cleanup separates the workflow run repository from the target PR repository"
	assert_file_contains "$workflow_file" 'repos/${RUN_REPOSITORY}/actions/runs' "strix cleanup queries the repository that owns each workflow run"
	...
	assert_file_contains "$workflow_file" "cancel-in-progress: \${{ github.event_name == 'push' || (github.event_name == 'pull_request_target' && (github.event.action == 'synchronize' || github.event.action == 'closed')) }}" "strix workflow cancels superseded push and synchronize/close scans while preserving review lifecycle evidence"
	...
	assert_file_contains "$workflow_file" "types: [strix-scan, strix-close-cleanup]" "strix repository dispatch accepts scan and leaf close-cleanup event types"
