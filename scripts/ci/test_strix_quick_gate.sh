...
assert_opencode_review_uses_codegraph_and_contextual_orchestrator() {
	...
	assert_file_contains "$opencode_config" '"model": "contextual-orchestrator/orchestrator/free"' "opencode review uses the gateway free pool"
	assert_file_contains "$opencode_config" '"small_model": "contextual-orchestrator/orchestrator/free"' "opencode review uses the gateway for the small model"
	assert_file_contains "$opencode_config" '"enabled_providers": ["contextual-orchestrator"]' "opencode review enables only the gateway provider"
	assert_file_contains "$opencode_config" '"baseURL": "{env:CONTEXTUAL_ORCHESTRATOR_BASE_URL}"' "opencode review routes model traffic through the gateway origin"
	assert_file_contains "$opencode_config" '"apiKey": "{env:CONTEXTUAL_ORCHESTRATOR_TOKEN}"' "opencode review routes model credentials through the gateway token"
	...
	assert_file_contains "$REPO_ROOT/ci-review-prompt.md" "EgressWeave policy enforcement and wardnet observation" "opencode review prompt limits network MCP access to governed owner paths"
	...
	assert_file_not_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" 'timeout --kill-after=30s "${run_timeout_seconds}s"' "opencode review model pool must not cap inference"
	...
	assert_file_not_contains "$workflow_file" 'OPENCODE_FREE_RUN_TIMEOUT_SECONDS:' "opencode free-tier review has no inference timeout"
	...
	assert_file_not_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" 'timeout --kill-after=30s' "opencode pool has no inference kill timer"
	assert_file_not_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" 'OPENCODE_NVIDIA_NIM_TOTAL_BUDGET_SECONDS' "opencode launcher has no direct NVIDIA NIM runtime policy"

	assert_file_not_contains "$workflow_file" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS:' "opencode model pool has no wall-clock retry budget"
	...
	assert_file_contains "$opencode_config" '"model": "contextual-orchestrator/orchestrator/free"' "opencode review starts the gateway model pool"
	assert_file_contains "$opencode_config" '"small_model": "contextual-orchestrator/orchestrator/free"' "opencode review uses the gateway small model"
	assert_file_contains "$opencode_config" '"enabled_providers": ["contextual-orchestrator"]' "opencode review uses the central gateway-only provider set"
	...
	assert_file_contains "$opencode_config" '"ci-review-fallback"' "opencode review declares a dedicated fallback agent"
	assert_file_contains "$opencode_config" '"steps": 150' "opencode review fallback preserves the established review budget after centralization"
	assert_file_contains "$opencode_config" '"lsp": false' "opencode review disables LSP in the central runtime config"
	assert_file_contains "$opencode_config" '"read": "allow"' "opencode review allows read-only file inspection"
	assert_file_contains "$opencode_config" '"grep": "allow"' "opencode review allows focused literal searches"
	...
	assert_file_not_contains "$workflow_file" 'timeout-minutes: 36' "opencode approval step must not cap model diagnosis"
	assert_file_not_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS:' "opencode publish-stage diagnosis has no inference timeout"
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" "assert_gateway_model_candidate" "opencode model pool validates the gateway-owned model boundary"
	assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" "bypasses the required contextual-orchestrator/orchestrator/free" "opencode model pool fails closed on direct-provider candidates"
	assert_file_not_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" "OPENAI_API_KEY is not configured" "opencode launcher has no direct OpenAI credential fallback"
	assert_file_not_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" "OPENROUTER_API_KEY is not configured" "opencode launcher has no direct OpenRouter credential fallback"
	assert_file_not_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" "scoped NVIDIA_NIM_API_KEY is not configured" "opencode launcher has no direct NVIDIA NIM credential fallback"
	...
	assert_file_not_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS' "opencode model pool has no wall-clock retry budget"
	...
	assert_file_not_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS:' "opencode catalog fallback permits arbitrarily slow provider sessions"
	...
	assert_file_contains "$opencode_config" '"enabled_providers": ["contextual-orchestrator"]' "opencode review keeps the central provider set gateway-only"
	assert_file_contains "$opencode_config" '"model": "contextual-orchestrator/orchestrator/free"' "opencode review keeps the central model on orchestrator/free"
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" '"contextual-orchestrator/orchestrator/free"' "schema repair remains restricted to the gateway-owned free pool"
	...
	assert_file_contains "$opencode_config" '"enabled_providers": ["contextual-orchestrator"]' "opencode review keeps model execution gateway-only"
	assert_file_contains "$opencode_config" '"baseURL": "{env:CONTEXTUAL_ORCHESTRATOR_BASE_URL}"' "opencode review binds the gateway origin in central config"
	assert_file_contains "$opencode_config" '"apiKey": "{env:CONTEXTUAL_ORCHESTRATOR_TOKEN}"' "opencode review binds the gateway token in central config"
	assert_file_not_contains "$workflow_file" "github-models/" "opencode review has no direct GitHub Models candidates"
	assert_file_not_contains "$workflow_file" "openai/gpt-" "opencode review has no direct OpenAI candidates"
	assert_file_not_contains "$workflow_file" "nvidia-nim/" "opencode review has no direct NVIDIA candidates"
	assert_file_not_contains "$workflow_file" "opencode-free/" "opencode review has no direct anonymous-provider candidates"
	...
	assert_file_contains "$REPO_ROOT/opencode.jsonc" '"graphify": {' "opencode config exposes only the prepared local Graphify MCP route"
	...
	assert_file_contains "$REPO_ROOT/ci-review-prompt.md" "The model is intentionally isolated from execution and direct network access." "opencode checked-in prompt documents the direct-network boundary"
	...
	assert_file_contains "$opencode_config" '"lsp": false' "opencode central config disables built-in LSP servers"
	assert_file_contains "$opencode_config" '"graphify": {' "opencode central config limits runtime MCP to the prepared Graphify graph"
	...
	assert_file_contains "$opencode_config" '"graphify": {' "opencode config exposes the prepared local Graphify graph"
	assert_file_contains "$opencode_config" '"command": ["graphify-mcp", "graphify-out/graph.json"]' "opencode Graphify route uses the prepared graph artifact"
	...
	assert_file_not_contains "$opencode_config" '"nvidia-nim"' "opencode config no longer defines a dormant nvidia-nim provider block"
	assert_file_not_contains "$opencode_config" 'integrate.api.nvidia.com' "opencode config no longer points at the NVIDIA NIM API"
	assert_file_not_contains "$opencode_config" '"github-models": {' "opencode config omits dormant direct GitHub Models routing"
	assert_file_not_contains "$opencode_config" '"openai": {' "opencode config omits dormant direct OpenAI routing"
	assert_file_not_contains "$opencode_config" '"openrouter": {' "opencode config omits dormant direct OpenRouter routing"
	assert_file_contains "$opencode_config" '"orchestrator/free": {' "opencode config delegates the only model pool to contextual-orchestrator"
	assert_file_contains "$opencode_config" '"context": 200000' "opencode config declares the orchestrator/free context window"
	assert_file_contains "$opencode_config" '"output": 32768' "opencode config declares the orchestrator/free output window"
	assert_file_contains "$opencode_config" '"reasoningEffort": "high"' "opencode config keeps high reasoning effort for capable review models"
}
...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	local core_scheduler_file="$REPO_ROOT/scripts/ci/pr_review_merge_scheduler_core.py"
	...
	assert_file_contains "$workflow_file" "github.event_name == 'pull_request_target' || (github.event_name == 'repository_dispatch' && github.event.client_payload.update_branches != false) || inputs.update_branches == true" "scheduler enables branch updates after native PR events or an explicit dispatch"
	...
	assert_file_not_contains "$workflow_file" "uses: actions/checkout" "scheduler does not use checkout in privileged pull_request_target contexts"
	...
	assert_file_contains "$core_scheduler_file" "expected_head_sha={head}" "scheduler guards branch updates with the current PR head SHA"
	assert_file_contains "$core_scheduler_file" "squash is disabled; retrying" "scheduler logs and retries with merge commit when repository settings reject squash"
	assert_file_contains "$core_scheduler_file" 'merge_args.extend(["--merge", "--match-head-commit", head])' "scheduler preserves the exact-head guard when falling back from squash"
	assert_file_contains "$core_scheduler_file" "shell=False" "scheduler subprocess wrapper forbids shell command execution"
	assert_file_contains "$core_scheduler_file" "check=True" "scheduler subprocess wrapper raises on failed commands"
	...
	assert_file_contains "$core_scheduler_file" "dispatch_strix_evidence" "scheduler dispatches same-head Strix evidence before OpenCode review"
	assert_file_contains "$core_scheduler_file" '"--method"' "scheduler reads active workflow runs with GET query parameters"
	assert_file_contains "$core_scheduler_file" "--security-workflow" "scheduler allows the canonical Strix workflow name to be configured"
	assert_file_contains "$core_scheduler_file" "same-head OpenCode dispatched" "scheduler records review dispatch after completed security evidence"
