...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	assert_file_contains "$workflow_file" 'REVIEW_DISPATCH_LIMIT must be explicitly configured' "scheduler fails closed when no review dispatch authority is configured"
	assert_file_contains "$workflow_file" 'BRANCH_UPDATE_LIMIT must be explicitly configured' "scheduler fails closed when no branch-update authority is configured"
	assert_file_contains "$workflow_file" 'REVIEW_ADMISSION_DISPATCH_BUDGET must be explicitly configured' "scheduler fails closed when no admission authority is configured"
	assert_file_not_contains "$workflow_file" 'review_dispatch_limit="1"' "scheduler does not invent a one-dispatch fallback"
