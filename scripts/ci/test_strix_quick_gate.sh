...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "STRIX_SCAN_MODE: \${{ github.event_name == 'schedule' && 'standard' || github.event_name == 'push' && (github.ref == 'refs/heads/main' || github.ref == 'refs/heads/master') && 'standard' || 'quick' }}" "strix workflow maps dual-flow events to official scan modes"
	...
	assert_file_contains "$workflow_file" "timeout-minutes: 120" "strix workflow keeps the 120-minute job budget for quick and standard"
	assert_file_contains "$workflow_file" "timeout-minutes: 100" "strix workflow keeps the 100-minute step budget for quick and standard"
	assert_file_not_contains "$workflow_file" "timeout-minutes: 360" "strix workflow must not apply the deep job ceiling"
	assert_file_not_contains "$workflow_file" "timeout-minutes: 340" "strix workflow must not apply the deep step ceiling"
	...
	assert_file_not_contains "$workflow_file" 'process_budget_seconds="14400"' "strix privileged workflow must not raise a 4-hour deep process budget"
	assert_file_not_contains "$workflow_file" 'total_budget_seconds="16200"' "strix privileged workflow must not raise a 4.5-hour deep total budget"
