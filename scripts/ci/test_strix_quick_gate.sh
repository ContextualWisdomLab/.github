...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	assert_file_contains "$workflow_file" "toJSON(github.event.client_payload.trigger_reviews) != 'false' && toJSON(github.event.client_payload.trigger_reviews) != '\"false\"'" "scheduler enables review dispatch by default for default-branch dispatch events"
	...
	assert_file_contains "$workflow_file" "toJSON(github.event.client_payload.enable_auto_merge) != 'false' && toJSON(github.event.client_payload.enable_auto_merge) != '\"false\"'" "scheduler enables auto-merge by default for default-branch dispatch events"
	assert_file_contains "$workflow_file" "github.event_name == 'pull_request_target' || (github.event_name == 'repository_dispatch' && toJSON(github.event.client_payload.update_branches) != 'false' && toJSON(github.event.client_payload.update_branches) != '\"false\"') || inputs.update_branches == true" "scheduler enables branch updates after native PR events or an explicit dispatch"
