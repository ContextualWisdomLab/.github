...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 120' "opencode approval step has a bounded wall-clock timeout that still permits a deep failed-check diagnosis"
	assert_file_contains "$workflow_file" 'focused failed-check diagnosis one bounded 90-minute single-model' "opencode failed-check diagnosis has enough time to produce useful blocker evidence without holding the queue for hours"
