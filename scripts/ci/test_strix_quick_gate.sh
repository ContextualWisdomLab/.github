...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_not_contains "$REPO_ROOT/.github/workflows/strix.yml" 'statuses: write' "strix manual evidence status keeps GITHUB_TOKEN commit-status permission read-only"
	...
	assert_file_not_contains "$REPO_ROOT/.github/workflows/strix.yml" 'post_strix_status "github-token" "$GITHUB_STATUS_TOKEN"' "strix manual evidence status does not depend on a GITHUB_TOKEN commit-status fallback"
