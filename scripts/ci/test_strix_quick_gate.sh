...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'gh api "repos/${CENTRAL_WORKFLOW_REPOSITORY}"' "opencode deferred review retry resolves the central workflow repository default branch"
	assert_file_contains "$workflow_file" '--ref "$central_workflow_ref"' "opencode deferred review retry dispatches from the resolved central workflow ref"
	assert_file_not_contains "$workflow_file" "--ref main" "opencode workflow dispatch does not hard-code either repository default branch"
