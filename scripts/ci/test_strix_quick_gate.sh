...
EOF
	...
	assert_file_contains "$REPO_ROOT/.github/workflows/opencode-review.yml" "scripts/ci/opencode_source_line_receipts.py" "opencode workflow generates trusted current-head source-line receipts"
	assert_file_contains "$REPO_ROOT/.github/workflows/opencode-review.yml" 'append_evidence_section "Trusted changed-line source receipts" 12000' "opencode workflow repeats trusted source-line receipts for models without file reads"
	assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" "Never calculate, infer, or invent a digest" "opencode model pool requires copying trusted source-line receipts"
