...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "github_models/openai/o3 github_models/openai/gpt-4o" "strix workflow keeps GitHub Models fallback on tool-capable OpenAI models without GPT-4.1 downgrade"
	assert_file_contains "$workflow_file" "steps.gate.outputs.provider_mode == 'openai_direct' && 'github_models/openai/o3 github_models/openai/gpt-4o'" "strix workflow gives direct-OpenAI scans GitHub Models fallbacks so provider quota outages degrade instead of skipping"
	assert_file_contains "$workflow_file" "steps.gate.outputs.provider_mode == 'nvidia_nim' && 'nvidia_nim/nvidia/llama-3.3-nemotron-super-49b-v1.5 github_models/openai/o3 github_models/openai/gpt-4o'" "strix workflow gives NVIDIA NIM scans contracted fallbacks"
...
assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" 'OPENCODE_NVIDIA_NIM_TOTAL_BUDGET_SECONDS 900' "opencode NVIDIA NIM combined runtime cap defaults to fifteen minutes"
	...
	assert_file_contains "$workflow_file" "github-models/openai/gpt-4o" "opencode review includes GitHub Models GPT-5 chat as a catalog fallback"
...
assert_file_contains "$opencode_config" 'integrate.api.nvidia.com' "opencode config points nvidia-nim at NIM API"
	...
	assert_file_contains "$opencode_config" '"openai/gpt-4o"' "opencode config defines GPT-5 Chat catalog fallback"
