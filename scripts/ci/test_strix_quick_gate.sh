...
assert_opencode_review_uses_codegraph_and_contextual_orchestrator() {
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/opencode_failure_envelope.py" "OpenCode provider failure metadata" "opencode review labels provider failure classes in the check log"
	assert_file_contains "$REPO_ROOT/scripts/ci/opencode_failure_envelope.py" "provider-controlled content suppressed" "opencode provider failure logging suppresses credential-bearing content"
