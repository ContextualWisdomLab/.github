...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 120' "opencode approval publication gives failed-check diagnosis deep-review scale time while remaining bounded"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 120' "opencode approval step has a bounded wall-clock timeout that still allows deep failed-check diagnosis"
	assert_file_contains "$workflow_file" 'focused failed-check diagnosis a deep single-model window' "opencode failed-check diagnosis has enough time to produce useful blocker evidence"
	...
	assert_file_contains "$workflow_file" 'APPROVAL_CHECK_WAIT_ATTEMPTS: "120"' "opencode approval waits long enough for slow peer checks before approving"
	assert_file_contains "$workflow_file" 'APPROVAL_CHECK_WAIT_SLEEP_SECONDS: "30"' "opencode approval poll cadence keeps peer-check API volume bounded"
	...
	assert_file_contains "$workflow_file" 'REVIEW_PUBLISH_RETRY_ATTEMPTS: "2"' "opencode approval gives review publication a bounded retry budget"
