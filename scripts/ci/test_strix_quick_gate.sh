...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "scan_stage_head_sha" "strix workflow falls back to the scan-stage head SHA only when run metadata is absent"
...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" "opencode_timeout_command=(" "opencode review model pool launches the timeout executable directly for reliable PID tracking"
	assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" "signal_process_tree" "opencode review model pool cleans up the timeout process tree and process group"
	...
	assert_file_contains "$REPO_ROOT/.github/workflows/strix.yml" "Strix evidence incomplete" "strix wrapper fails closed when provider failure prevents a completed report"
	assert_file_not_contains "$REPO_ROOT/.github/workflows/strix.yml" "neutral skip so an infrastructure outage does not block merges" "strix wrapper never converts missing security evidence into a pass"
