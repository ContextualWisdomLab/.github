...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 350' "opencode model pool keeps a bounded job timeout while leaving approval headroom"
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "2400"' "opencode primary review has a bounded per-model timeout before trying fallback models"
	assert_file_contains "$workflow_file" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "7200"' "opencode model pool has a script-level retry budget below the job timeout"
	...
	assert_file_contains "$workflow_file" "openai/gpt-5-mini openai/gpt-5 github-models/openai/o4-mini github-models/openai/o3-mini github-models/openai/gpt-5-mini" "opencode review tries native OpenAI before GitHub Models fallbacks"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "2400"' "opencode catalog fallback has a bounded model review timeout before step timeout"
	...
	assert_file_contains "$workflow_file" "github-models/openai/gpt-5-chat github-models/deepseek/deepseek-r1-0528 github-models/deepseek/deepseek-r1" "opencode review keeps reasoning catalog fallbacks after compact attempts"
