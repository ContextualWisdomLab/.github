...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" '"apiKey": "{env:NVIDIA_NIM_API_KEY}"' "opencode review resolves NVIDIA NIM credentials at runtime"
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" "NVIDIA_NIM_API_KEY is not configured" "opencode model pool skips NVIDIA NIM candidates when the org secret is absent"
