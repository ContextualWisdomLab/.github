...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "approve_current_head_after_model_unavailable" "opencode approval can use deterministic current-head evidence after model-output failures"
	...
	assert_file_contains "$workflow_file" "This fallback does not suppress failed checks, medium-or-higher code-scanning alerts, merge conflicts, unresolved reviewer threads, or failed coverage evidence" "opencode model-unavailable fallback stays gated by authoritative current-head evidence"
