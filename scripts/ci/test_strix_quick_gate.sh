...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "timeout-minutes: 360" "strix workflow job budget preserves multi-hour scans up to the GitHub-hosted runner job execution ceiling"
	assert_file_contains "$workflow_file" "timeout-minutes: 330" "strix workflow scan step permits legitimate 310-minute repository reviews"
	...
	assert_file_contains "$workflow_file" 'export "STRIX_TOTAL_${budget_suffix}_SECONDS=18900"' "strix workflow preserves a 315-minute bounded total Strix budget"
	assert_file_contains "$workflow_file" 'process_budget_seconds="18600"' "strix workflow gives a legitimate scan up to 310 minutes"
...
assert_opencode_review_uses_codegraph_and_contextual_orchestrator() {
	...
	# The range must end at the *next job key* (2-space indent), not the next
	# column-0 line: a GitHub Actions job list under `jobs:` never dedents to
	# column 0 until EOF, so `/^[^ ]/` as an end pattern silently captured
	# every line through the end of the file -- including unrelated later
	# jobs' own `if:` conditions (e.g. opencode-review-target's own
	# closed-PR skip) -- as if they belonged to required-workflow-bootstrap.
	if awk '
		/^  required-workflow-bootstrap:$/ { in_job = 1; next }
