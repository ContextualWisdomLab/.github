...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$REPO_ROOT/docs/doctoring/strix-scan-mode-dual-flow.md" "Write access is the only control" "strix doctoring records the workflow_dispatch revision-selection residual"
	assert_file_contains "$REPO_ROOT/docs/doctoring/strix-scan-mode-dual-flow.md" "gh workflow run --ref" "strix doctoring names the GitHub manual-run revision selector"
	assert_file_contains "$REPO_ROOT/docs/doctoring/strix-scan-mode-dual-flow.md" "Do not add \`target_repository\` or \`pr_number\` inputs" "strix doctoring forbids widening workflow_dispatch into a privileged retry"
