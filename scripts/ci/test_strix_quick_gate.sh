...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	assert_file_contains "$workflow_file" "github.event_name == 'repository_dispatch' && format('repo-dispatch-{0}', github.repository)" "scheduler deduplicates unscoped manual queue scans per repository"
	assert_file_not_contains "$workflow_file" "github.event_name == 'repository_dispatch' && github.run_id" "scheduler does not isolate duplicate unscoped manual scans by run ID"
	assert_file_contains "$workflow_file" "github.event_name == 'workflow_run' && !github.event.workflow_run.pull_requests[0].number && format('workflow-run-no-pr-{0}', github.repository)" "scheduler deduplicates metadata-free workflow completions per repository"
	assert_file_contains "$workflow_file" "cancel-in-progress: \${{ github.event_name == 'pull_request_target' || github.event_name == 'pull_request_review' || github.event_name == 'repository_dispatch' || (github.event_name == 'workflow_run' && !github.event.workflow_run.pull_requests[0].number) }}" "scheduler cancels stale PR/review/manual and metadata-free workflow scans instead of accumulating merge/update attempts"
