...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'repository: ${{ github.event.pull_request.base.repo.full_name }}' "opencode coverage checks out the PR merge ref from the base repository separately from trusted scripts"
	...
	assert_file_contains "$workflow_file" 'token: ${{ secrets.OPENCODE_APPROVE_TOKEN || github.token }}' "manual coverage evidence checks out the merge ref without an OIDC-exchanged app token"
	assert_file_contains "$workflow_file" 'ref: refs/pull/${{ github.event.pull_request.number }}/merge' "coverage evidence checks out the current pull request merge ref as data"
	assert_file_contains "$workflow_file" 'ref: refs/pull/${{ github.event.inputs.pr_number }}/merge' "manual coverage evidence checks out the requested pull request merge ref as data"
