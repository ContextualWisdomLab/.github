...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	assert_file_contains "$workflow_file" "github.event_name == 'repository_dispatch' && format('repo-dispatch-{0}', github.event.client_payload.target_repository || github.repository)" "scheduler keeps manual queue scans isolated per repository dispatch target"
