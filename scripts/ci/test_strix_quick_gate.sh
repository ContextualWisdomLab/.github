...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_not_contains "$workflow_file" "OPENCODE_POOL_MAX_CYCLES" "opencode model pool retries invalid control output until the bounded retry or step timeout fires"
	assert_file_contains "$workflow_file" 'OPENCODE_POOL_CYCLE_SLEEP_SECONDS: "15"' "opencode model pool uses a bounded sleep between invalid-output retry cycles"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "5400"' "opencode catalog fallback has enough per-model time for deep tool-using review before step timeout"
	...
	assert_file_contains "$workflow_file" "github-models/openai/gpt-5-chat" "opencode review keeps GPT-5 chat fallback coverage after compact reasoning attempts"
	assert_file_contains "$workflow_file" "github-models/openai/o3" "opencode review keeps o3 fallback coverage after compact reasoning attempts"
	assert_file_contains "$workflow_file" "github-models/deepseek/deepseek-r1-0528 github-models/deepseek/deepseek-r1 github-models/deepseek/deepseek-v3-0324" "opencode review keeps DeepSeek fallback coverage after OpenAI candidates"
