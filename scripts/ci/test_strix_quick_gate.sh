...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 350' "opencode review target keeps a bounded runner budget so stalled reviews release queue capacity"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 12' "opencode model pool fails closed quickly enough to keep the review queue moving"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "180"' "opencode primary review has a bounded per-model timeout for stalled providers"
	assert_file_contains "$workflow_file" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "540"' "opencode model pool exits before the job timeout so the approval gate can publish a reason"
	...
	assert_file_contains "$workflow_file" "openai/gpt-5 github-models/openai/gpt-5 github-models/openai/gpt-5-chat github-models/openai/o3" "opencode review starts with native OpenAI before GitHub Models fallbacks"
	...
	assert_file_contains "$workflow_file" 'APPROVAL_CHECK_WAIT_ATTEMPTS: "49"' "opencode approval waits for bounded long-running peer checks before approving"
	assert_file_contains "$workflow_file" 'APPROVAL_CHECK_WAIT_SLEEP_SECONDS: "15"' "opencode approval poll cadence keeps peer-check waits bounded"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "180"' "opencode catalog fallback has a bounded per-model review timeout before step timeout"
	...
	assert_file_contains "$workflow_file" "openai/gpt-5 github-models/openai/gpt-5 github-models/openai/gpt-5-chat github-models/openai/o3" "opencode review tries native OpenAI before GitHub Models fallbacks"
	assert_file_contains "$workflow_file" "github-models/openai/o3 github-models/deepseek/deepseek-r1-0528" "opencode review keeps reasoning and DeepSeek fallback coverage after OpenAI candidates"
	...
	assert_file_contains "$workflow_file" "pull-requests: write" "opencode approval can use github-actions[bot] for same-repository mechanical merge/update follow-up"
	...
	assert_file_contains "$workflow_file" "&& 'github-token' || secrets.PR_REVIEW_MERGE_TOKEN" "opencode scheduler follow-up prefers github-actions[bot] for same-repository mechanical mutations"
	...
	assert_file_not_contains "$REPO_ROOT/.github/workflows/strix.yml" 'post_strix_status "github-token" "$GITHUB_STATUS_TOKEN"' "strix manual evidence status avoids same-repository GITHUB_TOKEN status writes"
	...
	assert_file_contains "$workflow_file" 'post_pull_review_with_retry "primary review"' "opencode approval retries primary review publication before failing closed"
	assert_file_contains "$workflow_file" 'post_pull_review_with_retry "fallback review"' "opencode approval retries fallback review publication before failing closed"
	...
	assert_file_contains "$workflow_file" 'APPROVE_PUBLICATION_FAILED' "opencode approval fails closed when GitHub rejects an APPROVE review write"
	...
	assert_file_contains "$workflow_file" "github-models/openai/o3" "opencode review has a reachable o3 reasoning fallback model"
	...
	assert_file_contains "$workflow_file" 'FAILED_CHECK_EVIDENCE_SLEEP_SECONDS: "10"' "opencode review workflow retries peer-check evidence without stalling the model stage for Strix-scale durations"
	assert_file_contains "$workflow_file" 'FAILED_CHECK_EVIDENCE_COLLECT_TIMEOUT_SECONDS: "45"' "opencode review caps each failed-check evidence collection attempt"
	assert_file_contains "$workflow_file" 'Collecting failed-check evidence with %ss timeout' "opencode evidence logs failed-check collection timeout budgets"
	assert_file_contains "$workflow_file" 'non-Strix peer checks still running' "opencode evidence does not wait behind long-running Strix checks before model review"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_MODEL_CANDIDATES: "openai/gpt-5 github-models/openai/gpt-5 github-models/openai/gpt-5-chat github-models/openai/o3 github-models/deepseek/deepseek-r1-0528"' "opencode review uses the bounded high-sensitivity model pool"
	assert_file_contains "$workflow_file" "github-models/openai/o3" "opencode review includes GitHub Models o3 as a reasoning fallback"
...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	assert_file_contains "$workflow_file" "Materialize trusted scheduler" "scheduler materializes the canonical implementation without a privileged checkout action"
	assert_file_contains "$workflow_file" 'git fetch --depth=1 trusted "$TRUSTED_REF"' "scheduler fetches only the resolved central ref"
	assert_file_contains "$workflow_file" "Unsafe trusted scheduler source ref" "scheduler rejects unsafe central refs before materializing code"
	assert_file_not_contains "$workflow_file" "uses: actions/checkout" "scheduler avoids privileged checkout actions in pull_request_target/workflow_run contexts"
...
EOF
	...
	assert_file_contains "$REPO_ROOT/.github/workflows/opencode-review.yml" "OPENCODE_CHANGED_FILES_FILE<<OPENCODE_CHANGED_FILES_FILE_EOF" "opencode workflow writes changed-file env path with a GITHUB_ENV heredoc"
