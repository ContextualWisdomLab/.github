...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 90' "opencode model pool leaves approval-gate headroom while capping stalled model attempts"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "5400"' "opencode model pool exits before the job timeout so the approval gate can publish a reason"
