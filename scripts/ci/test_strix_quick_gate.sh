...
assert_opencode_review_uses_codegraph_and_contextual_orchestrator() {
	...
	assert_file_contains "$REPO_ROOT/ci-review-prompt.md" "A network MCP is allowed only when the" "opencode review prompt permits network MCP only through the governed central contract"
