...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "github.event_name == 'repository_dispatch' && github.run_id" "strix gives every pre-validation repository dispatch an isolated queue group"
	...
	assert_file_contains "$workflow_file" "cancel-in-progress: \${{ github.event_name != 'repository_dispatch' }}" "strix never lets an unvalidated stale repository dispatch cancel newer evidence"
	...
	assert_file_contains "$workflow_file" "run-id scope prevents an out-of-order stale dispatch" "strix workflow documents stale dispatch isolation"
...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	assert_file_contains "$workflow_file" "github.event.client_payload.agent_invocation_key != '' && format('agent-review-{0}', github.event.client_payload.agent_invocation_key)" "scheduler isolates each validated agent-review invocation before live snapshot validation"
	assert_file_contains "$workflow_file" "github.event.action != 'merge-scheduler-agent-review-v2' && github.event.client_payload.target_repository" "scheduler retains target-PR cancellation only for legacy dispatches"
	...
	assert_file_contains "$workflow_file" "github.event_name == 'workflow_run' || (github.event_name == 'repository_dispatch' && github.event.action != 'merge-scheduler-agent-review-v2' && github.event.client_payload.update_branches != false) || inputs.update_branches == true" "scheduler enables branch updates after review completion or an explicit legacy default-branch dispatch while keeping agent review v2 disabled"
