...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" 'repository: ContextualWisdomLab/.github' "strix workflow checks out central Strix scripts instead of target-repo copies"
	assert_file_contains "$workflow_file" 'ref: ${{ github.workflow_sha }}' "strix workflow checks out the exact trusted Strix workflow SHA"
...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	assert_file_contains "$workflow_file" 'ref: ${{ github.workflow_sha }}' "scheduler checks out the resolved central workflow SHA"
