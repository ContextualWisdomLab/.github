...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "pull_request:" "opencode review workflow uses the ruleset-supported unprivileged PR event"
	...
	if grep -Eq '^[[:space:]]+pull_request_target:[[:space:]]*$' "$workflow_file"; then
		record_failure "opencode review workflow must not run PR-controlled coverage in pull_request_target"
	fi
	...
	assert_file_contains "$workflow_file" 'TARGET_REPOSITORY: ${{ github.event.pull_request.base.repo.full_name || github.event.inputs.target_repository || github.repository }}' "opencode pull_request coverage fetches exact base/head commits from the target repository"
	...
	assert_file_contains "$workflow_file" "LEGACY_GITHUB_ACTIONS_REVIEW_TOKEN: \${{ github.event_name == 'pull_request' && github.token || '' }}" "opencode app-token approval can bridge stale same-repo github-actions review state"
	...
	assert_file_contains "$workflow_file" 'SCHEDULER_READ_TOKEN: ${{ (github.event_name == '\''pull_request'\'' || needs.validate-pr-metadata.outputs.target_repository == github.repository) && github.token || secrets.PR_REVIEW_MERGE_TOKEN || secrets.OPENCODE_APPROVE_TOKEN || steps.opencode_app_token.outputs.token }}' "opencode scheduler follow-up reads cross-repository PR state with target-capable credentials"
