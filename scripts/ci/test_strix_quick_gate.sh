...
assert_strix_evidence_binding_contract() {
	...
	assert_file_contains "$GATE_SCRIPT" 'local binder="$SCRIPT_DIR/strix_evidence_binding.py"' "strix gate loads the binder from the control-plane script dir, not STRIX_REPO_ROOT"
...
assert_opencode_review_uses_codegraph_and_contextual_orchestrator() {
	...
	assert_file_contains "$workflow_file" "Folded together with the former coverage-source-tree job" "opencode workflow materializes coverage source before running PR-head tests"
	...
	assert_file_contains "$workflow_file" "Coverage merge tree could not be materialized" "coverage evidence logs source materialization failures as the coverage blocker"
