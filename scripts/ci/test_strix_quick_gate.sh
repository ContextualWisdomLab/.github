...
assert_strix_workflow_pr_trigger_hardened() {
	...
	fallback_line="$(grep -F 'STRIX_FALLBACK_MODELS:' "$workflow_file")"
	if grep -Fq "steps.gate.outputs.provider_mode == 'openai_direct' &&" <<<"$fallback_line"; then
		record_failure "strix workflow must leave direct-OpenAI fallback selection to the final empty default"
	fi
