...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 150' "opencode model pool keeps retrying with bounded queue impact while leaving approval headroom"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "8400"' "opencode model pool exits before the job timeout so the approval gate can publish a reason"
