...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" 'Provider outages without a vulnerability report are incomplete security evidence and are not converted to success' "strix workflow keeps provider exhaustion fail-closed"
	assert_file_not_contains "$workflow_file" "Treating as a neutral skip" "strix workflow must not convert provider exhaustion into a successful security check"
	assert_file_not_contains "$workflow_file" "backend_unavailable_signal=" "strix workflow must not recognize provider errors in order to downgrade the gate exit code"
	...
	assert_file_contains "$workflow_file" "steps.gate.outputs.provider_mode == 'nvidia_nim' && 'nvidia_nim/nvidia/llama-3.1-nemotron-ultra-253b-v1 nvidia_nim/nvidia/nemotron-3-super-120b-a12b github_models/openai/o3 github_models/openai/gpt-5-chat'" "strix workflow tries same-provider NIM Nemotron fallbacks before degrading to GitHub Models"
...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$REPO_ROOT/.github/workflows/strix.yml" 'exit "$strix_rc"' "strix wrapper propagates provider-failure and finding exit codes without downgrading them"
