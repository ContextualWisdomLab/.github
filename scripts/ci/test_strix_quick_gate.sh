...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "cancel-in-progress: \${{ github.event_name == 'push' || github.event.action == 'synchronize' }}" "strix workflow cancels superseded pushes and PR heads but preserves same-head lifecycle evidence"
	assert_file_contains "$workflow_file" "'stable-pr'" "strix workflow coalesces replacement heads before provider admission"
