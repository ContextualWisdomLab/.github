...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 30' "opencode approval publication returns the runner promptly while preserving bounded failed-check diagnosis"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 30' "opencode approval step has a bounded wall-clock timeout that protects the organization queue"
	assert_file_contains "$workflow_file" 'focused failed-check diagnosis one bounded single-model window' "opencode failed-check diagnosis has enough time to produce useful blocker evidence without holding the queue for hours"
