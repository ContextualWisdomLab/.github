...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 60' "opencode approval publication gives failed-check diagnosis more than 30 minutes while remaining bounded"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 60' "opencode approval step has a bounded wall-clock timeout that still permits a focused failed-check diagnosis"
	assert_file_contains "$workflow_file" 'focused failed-check diagnosis one bounded 50-minute single-model' "opencode failed-check diagnosis has enough time to produce useful blocker evidence without holding the queue for hours"
	...
	assert_file_contains "$workflow_file" 'REVIEW_PUBLISH_STEP_TIMEOUT_SECONDS: "240"' "opencode review publication has a wall-clock watchdog"
	assert_file_contains "$workflow_file" "PUBLISH_STEP_TIMEOUT: OpenCode publish step exceeded" "opencode review publication timeout leaves an explicit log reason"
