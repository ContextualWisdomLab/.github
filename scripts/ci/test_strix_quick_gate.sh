...
assert_opencode_review_posts_suggested_diffs_inline() {
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/opencode_inline_comment_fallback.py" "GitHub did not accept the inline review comments" "opencode review explains anchor failures instead of copying diffs to the PR body"
	assert_file_contains "$REPO_ROOT/scripts/ci/opencode_inline_comment_fallback.py" "accepted some inline comments" "opencode mixed-success receipts distinguish attached and refused comments"
	...
	assert_file_contains "$workflow_file" "--record-attach" "opencode records attached path:line rows beside refused receipts"
	...
	assert_file_contains "$workflow_file" "--applyable-locations" "opencode persists applyable suggestion ranges in overview receipts"
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/opencode_inline_comment_fallback.py" "remap_left_comment_to_right_hunk" "opencode remaps applyable LEFT leftovers onto a same-path RIGHT hunk"
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/opencode_inline_comment_fallback.py" "range(start, end + 1)" "opencode leftover heading matches interior lines of a deferred path:start-end"
