...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "Materialize central Strix dependency lock from PR head" "strix workflow validates same-repository central dependency-lock PRs"
	assert_file_contains "$workflow_file" "github.event.pull_request.base.repo.full_name == 'ContextualWisdomLab/.github'" "strix PR-head lock override requires the central base repository"
	...
	assert_file_contains "$workflow_file" '"$lock_mode" != "100644"' "strix Dependabot lock validation requires a regular Git blob"
	assert_file_contains "$workflow_file" 'trusted_lock="$TRUSTED_STRIX_SOURCE/requirements-strix-ci-hashes.txt"' "strix workflow selects the hashed dependency lock from trusted source"
	assert_file_contains "$workflow_file" 'resolved_trusted_lock="$(realpath "$trusted_lock")"' "strix workflow resolves the trusted lock before installation"
	assert_file_contains "$workflow_file" '--require-hashes -r "$resolved_trusted_lock"' "strix workflow installs only the resolved trusted lock"
	...
	assert_file_contains "$workflow_file" "github.event.client_payload.strix_llm || 'gpt-5.6-luna'" "strix workflow defaults PR Strix scans to direct OpenAI GPT-5.6 Luna"
	...
	assert_file_contains "$workflow_file" "STRIX_LLM must select GitHub Models openai/gpt-5 or newer, direct OpenAI GPT-5.4 or newer, OpenRouter openrouter/free, or an approved organization Vertex AI model" "strix workflow rejects unsupported model inputs"
	...
	assert_file_contains "$GATE_SCRIPT" '[[ "$candidate" == "$existing" || "$candidate" == "$existing/"* ]]' "strix snapshot comparison keeps captured report paths literal"
	assert_file_not_contains "$GATE_SCRIPT" 'case "$candidate" in' "strix snapshot comparison does not interpret captured paths as case patterns"
...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "types: [opened, synchronize, reopened, ready_for_review, edited, closed]" "opencode required workflow reacts to current PR/base changes and closed-PR cleanup"
	...
	assert_file_contains "$workflow_file" "Materialize inert pull request blobs for OpenCode review data" "opencode review materializes PR-head blobs as inert read-only review data"
	assert_file_contains "$workflow_file" 'remote add pr-source' "opencode review fetches target PR commits through a separate PR-source remote"
	...
	assert_file_contains "$workflow_file" 'git init --bare "$OPENCODE_SOURCE_GIT_DIR"' "opencode review isolates PR objects from the trusted workflow checkout"
	assert_file_contains "$workflow_file" 'python3 scripts/ci/materialize_pr_review_source.py' "opencode review materializes only validated inert PR blobs"
	assert_file_not_contains "$workflow_file" 'git worktree add --detach "$OPENCODE_SOURCE_WORKDIR"' "opencode review never creates an untrusted PR worktree"
	assert_file_contains "$workflow_file" 'find "$OPENCODE_SOURCE_WORKDIR" -type l' "opencode review rejects materialized symbolic links"
	assert_file_contains "$workflow_file" 'find "$OPENCODE_SOURCE_WORKDIR" -type f -perm /111' "opencode review rejects materialized executable files"
	assert_file_contains "$workflow_file" 'cd "$OPENCODE_SOURCE_WORKDIR"' "opencode CodeGraph indexing runs against inert PR-head blobs"
	assert_file_contains "$workflow_file" 'PR_MERGE_BASE="$(git -C "$OPENCODE_SOURCE_WORKDIR" merge-base "$PR_BASE_SHA" "$PR_HEAD_SHA")"' "opencode review evidence diffs use the isolated bare object store merge base"
	assert_file_contains "$workflow_file" 'git -C "$OPENCODE_SOURCE_WORKDIR" diff' "opencode review builds changed-file evidence from the isolated bare object store"
	...
	assert_file_contains "$workflow_file" "github-models/openai/gpt-4.1 openai/gpt-5.6-luna openrouter/deepseek/deepseek-v3.2 openrouter/qwen/qwen3-coder github-models/deepseek/deepseek-v3-0324 github-models/openai/gpt-5" "opencode review starts with the responsive GPT-4.1 endpoint before bounded paid and provider fallbacks"
	...
	assert_file_contains "$workflow_file" "Cross-repository repository_dispatch approval hold" "cross-repository pending approvals avoid poisoning the central source-branch check"
	...
	assert_file_contains "$workflow_file" "Cross-repository repository_dispatch review-tool failure" "cross-repository dispatch tool failures log the reason without poisoning the central source-branch check"
	...
	assert_file_contains "$workflow_file" "github-models/openai/gpt-4.1 openai/gpt-5.6-luna openrouter/deepseek/deepseek-v3.2 openrouter/qwen/qwen3-coder github-models/deepseek/deepseek-v3-0324 github-models/openai/gpt-5" "opencode review tries responsive GPT-4.1 before bounded paid and DeepSeek fallbacks"
	...
	assert_file_contains "$workflow_file" '"$2" -m coverage run -m pytest tests' "opencode coverage runs Python tests with the base hash-locked environment"
	assert_file_contains "$workflow_file" '"$2" -m coverage report --show-missing' "opencode coverage preserves the missing-line report with the selected trusted environment"
	assert_file_contains "$workflow_file" '"$2" -m pytest tests/test_docstrings.py' "opencode docstring tests use the selected trusted environment"
	assert_file_contains "$workflow_file" "trusted base commit; missing imports still fail in pytest" "unavailable base dependencies fail closed with their import error"
	...
	assert_file_contains "$workflow_file" "github-models/openai/gpt-4.1 openai/gpt-5.6-luna openrouter/deepseek/deepseek-v3.2 openrouter/qwen/qwen3-coder github-models/deepseek/deepseek-v3-0324 github-models/openai/gpt-5" "opencode review starts with responsive GPT-4.1 before bounded paid and full-size fallbacks"
	...
	assert_file_contains "$workflow_file" 'local source_root="${OPENCODE_SOURCE_WORKDIR:-${GITHUB_WORKSPACE:-$PWD}}"' "opencode trusted-base Strix lag detection inspects inert PR-head blobs"
	assert_file_contains "$workflow_file" 'git -C "$source_root" diff --quiet' "opencode trusted-base Strix lag detection compares trusted-input changes in the isolated bare object store"
...
EOF
	...
	assert_file_contains "$REPO_ROOT/.github/workflows/opencode-review.yml" 'git -C "$OPENCODE_SOURCE_WORKDIR" diff --name-only --find-renames "$PR_MERGE_BASE" "$PR_HEAD_SHA" |' "opencode workflow derives exact changed files from the isolated bare object store"
