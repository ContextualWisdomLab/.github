...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "--require-hashes --only-binary=:all: -r requirements-opencode-review-ci-hashes.txt" "coverage tooling installs from a hash-pinned lock"
	...
	assert_file_contains "$workflow_file" 'OpenCode could not publish APPROVE pull review for head %s; keeping the successful approval gate because the durable Review Overview was updated and branch protection remains authoritative for required checks.' "opencode approval soft-passes approve review publication failures after updating overview evidence"
	assert_file_contains "$workflow_file" 'if [ "$event" = "APPROVE" ]; then' "opencode approval soft-pass is limited to APPROVE review publication failures"
	assert_file_contains "$workflow_file" 'OpenCode could not publish the pull review for head %s, so the review state was not changed.' "opencode request-changes/comment publication still fails closed when review publication fails"
	...
	assert_file_contains "$workflow_file" 'OpenCode approve review publication skipped' "opencode approval reports skipped approval review publication as a warning"
	assert_file_contains "$workflow_file" 'keeping the successful approval gate' "opencode approval soft-passes only after a successful approval gate"
	assert_file_not_contains "$workflow_file" 'gh_error_is_rate_limited()' "opencode approval soft-pass is not limited to a rate-limit-specific bypass"
