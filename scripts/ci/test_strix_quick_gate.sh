...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_not_contains "$workflow_file" "Materialize central Strix dependency lock from PR head" "strix workflow never executes dependency metadata from PR head"
	assert_file_not_contains "$workflow_file" 'PR_HEAD_SHA:requirements-strix-ci-hashes.txt' "strix workflow never reads the dependency lock from PR head"
