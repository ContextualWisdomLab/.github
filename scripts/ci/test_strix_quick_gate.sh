...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "format('pr-{0}', github.event.pull_request.number)" "opencode review scopes pull_request_target concurrency to the active PR so newer heads cancel stale review runs"
	assert_file_contains "$workflow_file" "format('pr-{0}', github.event.inputs.pr_number)" "opencode review scopes manual concurrency to the requested PR so newer heads cancel stale review runs"
