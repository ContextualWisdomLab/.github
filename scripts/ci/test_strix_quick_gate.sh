...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 75' "opencode model pool gives deep tool-using reviews a bounded window while capping stalled model attempts"
	assert_file_contains "$workflow_file" 'timeout-minutes: 35' "opencode approval publication releases queue capacity after a bounded gate window"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "900"' "opencode primary review gives each model 15 minutes before trying fallback models"
	assert_file_contains "$workflow_file" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "3600"' "opencode model pool exits before the job timeout so the approval gate can publish a reason"
	...
	assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS:-3600' "opencode model pool keeps a bounded default retry budget unless the workflow explicitly disables it"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "900"' "opencode catalog fallback has a 15-minute per-model review timeout before step timeout"
	...
	assert_file_contains "$workflow_file" 'REVIEW_PUBLISH_RETRY_MAX_SLEEP_SECONDS: "60"' "opencode approval caps review publication retry sleeps for queue health"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 75' "opencode model stage has a bounded but deep-review-capable timeout"
...
FAIL: opencode failed-check diagnosis keeps DeepSeek R1 (missing 'MODEL: github-models/deepseek/deepseek-r1-0528')
EOF
