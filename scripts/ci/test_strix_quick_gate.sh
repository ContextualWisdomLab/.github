...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" "Checkout pull request merge ref for coverage measurement" "opencode coverage measures the PR merge ref instead of exposing secrets to PR-head execution"
	assert_file_contains "$workflow_file" 'repository: ${{ github.event.pull_request.base.repo.full_name }}' "opencode pull_request_target coverage reads the trusted base repository merge ref"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 350' "opencode model pool has runner budget while the script deadline leaves approval headroom"
	...
	assert_file_contains "$workflow_file" "github-models/deepseek/deepseek-v3-0324 github-models/openai/o4-mini github-models/openai/o3-mini" "opencode review tries the observed high-success DeepSeek V3 path before compact OpenAI reasoning fallbacks"
	...
	assert_file_contains "$workflow_file" "Checkout pull request merge ref for coverage measurement" "required OpenCode reviews measure coverage instead of approving skipped coverage evidence"
	assert_file_contains "$workflow_file" "Checkout requested pull request merge ref for coverage measurement" "manual OpenCode reviews measure coverage instead of approving skipped coverage evidence"
	...
	assert_file_contains "$workflow_file" 'token: ${{ secrets.OPENCODE_APPROVE_TOKEN || github.token }}' "coverage evidence checks out PR head without an OIDC-exchanged app token"
