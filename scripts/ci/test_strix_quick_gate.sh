...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	assert_file_contains "$workflow_file" 'cron: "30 * * * *"' "scheduler wakes frequently enough to clear auto-merge PRs that become stale after their initial PR events, lengthened from */30 to hourly (offset from org-queue-sweep's own hourly tick) for the same Actions-capacity reason as #1630 (#1704)"
	assert_file_not_contains "$workflow_file" '*/30 * * * *' "scheduler's repository-local heartbeat must not regress back to the pre-#1704 quarter-hourly cadence"
