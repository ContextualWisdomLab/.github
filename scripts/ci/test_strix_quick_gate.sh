...
assert_strix_pr_scope_includes_contextual_orchestrator_context() {
	...
	assert_file_contains "$GATE_SCRIPT" 'git -c core.quotepath=false ls-tree -r --name-only "$contextual_orchestrator_head_sha" -- contextual_orchestrator' "strix gate enumerates contextual-orchestrator context from the exact PR head"
}
