...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" 'STRIX_MODEL: ${{ steps.resolve_model.outputs.strix_model }}' "strix workflow propagates the diversity-gate-resolved model to the scanner"
	...
	assert_file_contains "$workflow_file" "STRIX_LLM must select contextual-orchestrator/orchestrator/auto or contextual-orchestrator/orchestrator/free" "strix workflow accepts only the gateway model"
