...
assert_pr_review_merge_scheduler_uses_github_actions_bot_token() {
	...
	assert_file_not_contains "$workflow_file" "Resolve trusted scheduler source ref" "scheduler required workflow avoids dynamic trusted-ref checkout in privileged events"
	assert_file_not_contains "$workflow_file" "github.workflow_ref" "scheduler required workflow does not derive a checkout ref from event context"
