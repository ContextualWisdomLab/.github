...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 75' "opencode review target keeps a bounded runner budget so stalled reviews release queue capacity"
	...
	assert_file_contains "$workflow_file" 'timeout-minutes: 25' "opencode model pool leaves approval-gate headroom while capping stalled model attempts"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "300"' "opencode primary review has a bounded per-model timeout before trying fallback models"
	assert_file_contains "$workflow_file" 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "1200"' "opencode model pool exits before the job timeout so the approval gate can publish a reason"
	...
	assert_file_contains "$workflow_file" 'OPENCODE_RUN_TIMEOUT_SECONDS: "300"' "opencode catalog fallback has a bounded per-model review timeout before step timeout"
