...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_not_contains "$workflow_file" "  admit-current-head:" "strix workflow consolidates metadata admission into changed-scope"
	assert_file_contains "$workflow_file" "needs: [changed-scope]" "strix provider queue waits for consolidated live-head admission"
	assert_file_contains "$workflow_file" "needs.changed-scope.outputs.admitted == 'true'" "strix provider scan requires the consolidated admission output"
