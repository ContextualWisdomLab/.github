...
assert_file_contains "$REPO_ROOT/scripts/ci/run_opencode_review_model_pool.sh" 'OPENCODE_NVIDIA_NIM_TOTAL_BUDGET_SECONDS 900' "opencode NVIDIA NIM combined runtime cap defaults to fifteen minutes"
	...
	assert_file_contains "$workflow_file" '[ "$pnpm_major" -ge 11 ]' "coverage omits --trust-lockfile on pnpm major versions before 11"
	assert_file_contains "$workflow_file" "javascript_coverage_provider_declared()" "coverage does not invent a JavaScript coverage instrumenter the package did not declare"
