...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "--require-hashes --no-deps -r requirements-strix-ci-hashes.txt" "strix workflow installs the complete hashed lock without dependency re-resolution"
	assert_file_contains "$workflow_file" "validate_strix_runtime_compatibility.py" "strix workflow executes exact-pin and crypto consumer smoke checks"
	...
	assert_file_not_contains "$workflow_file" "Treating as a neutral skip" "strix workflow keeps incomplete provider scans fail-closed"
	assert_file_contains "$GATE_SCRIPT" "import-current-attempt" "strix gate imports authoritative current-attempt reports"
	assert_file_contains "$GATE_SCRIPT" "vulnerability_file_is_self_negating_no_finding" "strix gate semantically excludes contradictory no-finding records"
...
assert_opencode_review_uses_codegraph_and_gpt5_fallback() {
	...
	assert_file_not_contains "$REPO_ROOT/.github/workflows/strix.yml" "backend_unavailable_signal=" "strix wrapper does not neutralize incomplete provider scans"
	assert_file_not_contains "$REPO_ROOT/.github/workflows/strix.yml" "Treating as a neutral skip" "strix wrapper keeps provider outages fail-closed"
	assert_file_contains "$REPO_ROOT/.github/workflows/strix.yml" "Strix did not produce complete passing security evidence" "strix wrapper preserves incomplete-scan failures"
