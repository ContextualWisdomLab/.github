...
assert_strix_workflow_pr_trigger_hardened() {
	...
	assert_file_contains "$workflow_file" "cancel-in-progress: false" "strix workflow queues evidence runs instead of cancelling them"
	assert_file_not_contains "$workflow_file" "format('pr-{0}-{1}'" "strix workflow does not keep stale head-specific concurrency groups"
	assert_file_contains "$workflow_file" "queue: max" "strix workflow preserves every queued evidence run"
	...
	assert_file_contains "$workflow_file" "Serializing per repository keeps at most" "strix workflow documents repository-level queue management"
