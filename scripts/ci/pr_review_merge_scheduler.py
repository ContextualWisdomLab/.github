    ...
    while len(prs) < max_prs:
        ...
        if len(payload) <= 1:
            ...
            with concurrent.futures.ThreadPoolExecutor(
                ...
                prs.extend(
                    list(executor.map(lambda pr: rest_pr_node(repo, pr), payload))
                )
...
def restamp_pr_head_for_last_push_approval(
    ...
    if not same_repository_head(repo, pr):
        raise RuntimeError(
            "last-push approval head refresh only supports same-repository PR heads"
        )
...
def active_workflow_runs(
    repo: str, statuses: Sequence[str] = ("queued", "in_progress")
) -> list[dict[str, Any]]:
...
def _test_split_repo() -> None:
    """Test split_repo logic."""
...
def _test_inspect_pr() -> None:
    """Test inspect_pr logic on various states."""
    sample = {
        "number": 1,
        "headRefOid": "abc",
        "baseRefName": "main",
...
def _test_inspect_blocked_pr() -> None:
    """Test inspect_pr logic on a blocked PR."""
    blocked_sample = {
        "number": 2,
        "headRefOid": "abc",
        "baseRefName": "main",
...
def _test_contract_decision() -> None:
    """Test contract_decision mapping."""
...
def _test_decision_guidance() -> None:
    """Test decision_guidance mapping."""
...
def _test_decision_payload() -> None:
    """Test decision_payload mapping."""
...
def self_test() -> None:
    ...
    _test_split_repo()
    _test_inspect_pr()
    _test_inspect_blocked_pr()
    _test_contract_decision()
    _test_decision_guidance()
    _test_decision_payload()
