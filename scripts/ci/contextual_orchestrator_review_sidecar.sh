...
# Launcher runtime preflight is the sole live provider observation during
# provisioning. Replaying a chat/completions request here would manufacture a
# second inference, its own token/sampling policy, and a shell-local retry
# budget after the route has already been admitted. Healthz plus the persisted
# per-route preflight evidence are the sidecar readiness boundary; consumers
# exercise the OpenAI-compatible endpoint with their real workload.
