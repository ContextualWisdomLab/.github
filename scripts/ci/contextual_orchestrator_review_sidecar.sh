...
# "orchestration":"route" pins this smoke request to the direct virtual-pool
# dispatch path rather than the gateway's own auto-mode triage, so a failure
# here is unambiguously the orchestrator/free route itself, not a triage
