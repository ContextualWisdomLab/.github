...
def strip_overlapping_leftover_suggestions(
    ...
    """Remove closed GitHub suggestion fences that overlap leftover coverage.

    ``applyable.txt`` already omits leftover interiors, including leftover
    ``path:start-end`` rows. The posted review JSON must not still offer a
    one-click apply on those same leftover lines (CWE-1288: only a closed
    `` ```suggestion `` fence is stripped; a prose mention is not).
    """
