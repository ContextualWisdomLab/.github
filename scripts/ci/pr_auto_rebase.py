...
def evaluate_candidate(
    ...
    """Evaluate a single PR candidate and return the decision and updated rate-limit count."""
    ...
    if skip_reason is not None:
        return Decision(number, "skip", skip_reason), rebases_used
    if rebases_used >= args.max_per_run:
        return Decision(number, "skip", f"rate limit reached ({args.max_per_run} rebases/run); process next run"), rebases_used
    rebases_used += 1
    if args.dry_run:
        return Decision(number, "would_rebase", f"candidate ({candidate_state_note(pr)}); dry-run, no git mutation"), rebases_used
    try:
        return perform_rebase(args.repo, pr, dry_run=args.dry_run), rebases_used
    except (RuntimeError, KeyError, ValueError) as exc:
        return Decision(number, "error", summarize_error(exc)), rebases_used
...
def process_queue(args: argparse.Namespace) -> int:
    ...
    rebases_used = 0
    for pr in prs:
        decision, rebases_used = evaluate_candidate(args, pr, now, rebases_used)
