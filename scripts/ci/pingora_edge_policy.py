...
def _is_recognized_documentation_image(path: str, raw: bytes) -> bool:
    """Accept only bounded bytes with a structurally complete PNG envelope."""
...
def evaluate_pull_request(
    *,
    ...
    for changed in changed_files:
        if changed.status != "removed" and _is_documentation_image_path(changed.path):
            raw = _load_raw_file_bytes(
                api_url.rstrip("/"), repository, changed.path, head_sha, token, opener
            )
            if not _is_recognized_documentation_image(changed.path, raw):
                raise PolicyError(
                    f"Documentation image evidence for {changed.path} is not a recognized image"
                )
            continue
