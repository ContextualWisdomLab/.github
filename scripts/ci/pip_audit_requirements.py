...
_PIP_CONFIG_OPTION_HEADS = frozenset(
    {
        "--require-hashes",
        "--index-url",
        "--extra-index-url",
...
def _is_pip_config_option(line: str) -> bool:
    """Return whether a line only configures pip and does not add a package."""

    head = line.split(None, 1)[0]
    if "=" in head:
        head = head.split("=", 1)[0]
    return head in _PIP_CONFIG_OPTION_HEADS
...
def is_hashed_lock(path: pathlib.Path) -> bool:
    ...
    not sufficient. Resolver configuration such as ``--index-url`` is not a
    package line. Include, constraint, and editable lines remain package
    input and cannot earn resolver bypass. Every remaining package line must
    be an exact ``==`` pin carrying one or more complete SHA-256 hashes
    before ``--disable-pip`` is allowed to bypass pip's resolver.
    """

    lines = _requirement_lines(path)
    package_lines = [line for line in lines if not _is_pip_config_option(line)]
...
def discover_requirement_files(root: pathlib.Path) -> list[pathlib.Path]:
    ...
    for path in sorted(root.rglob("requirements*.txt")):
        ...
        _reject_escaped_requirement_path(root, path)
        found.append(path)
    return found
