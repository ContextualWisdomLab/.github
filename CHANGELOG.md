...
- Added head-matched OpenCode review gold corpus tooling with adjudication, shadow, and quality scoring.
...
- Bound head-matched OpenCode gold scoring to freeze SHA-256 receipts and source-anchored findings, allowed explicit no-defect freezes, required adjudicator output-hiding attestation, aligned the sampler default seed, and replaced symlink-following temporary writes with exclusive `O_NOFOLLOW` publication (CWE-367).
