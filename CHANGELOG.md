...
- Refreshed the read-only review-dispatch workflow blob contract after the protected main branch changed that workflow, keeping the independent reviewer key-system guard bound to the live canonical blob.
...
- Allow governed anonymous OpenCode free-model candidates on private repositories only when an unchanged trusted-base policy opts in, wrap `opencode` with a provider-scoped credential guard, and pass the trusted `validate-pr-metadata` visibility signal into the live model-pool runner.
