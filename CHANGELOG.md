...
- Added a dedicated Contextual Orchestrator hourly caller at minute 17 and a target-scoped OpenCode gateway route that bootstraps all five approved provider credentials through the KV seam, performs pinned-source auto-discovery, and verifies `/v1/models` before repair work.
