...
- Require OpenCode APPROVE to name every current-head changed file as a whole path token so a sibling such as `example.py.bak` cannot stand in for `example.py`.
