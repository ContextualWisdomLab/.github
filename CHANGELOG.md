...
- Bound `requirements-strix-ci-hashes.txt` to the permanent Strix changed-path quality workflow so a manifest-only lock update cannot skip the install, policy, and full-suite contract.
