...
- O(N) 서브스트링 검색 최적화: `scripts/ci/opencode_review_normalize_output.py`의 `label_section` 함수에서 `APPROVAL_VERIFICATION_LABELS` 반복 탐색 시 범위를 제한하여 O(N^2) 병목 현상을 방지하고 파싱 성능을 개선했습니다.
