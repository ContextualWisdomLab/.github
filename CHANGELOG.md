### CodeQL handler accepts nested rerun requests before producer cutover

- `codeql-scan-dispatch.yml` now accepts the nested
  `rerun_request: {mode, required_jobs}` envelope emitted by the proposed
  scheduler/producer while retaining the protected handler's legacy inputs.
- Supplying nested and legacy rerun authority together fails closed instead
  of choosing one representation implicitly.
- Rerun settlement now has one run-level owner after all scan shards finish,
  preventing sibling matrix jobs from issuing competing rerun requests for
  the same required-workflow attempt. This handler-only predecessor must land
  before the producer cutover in `.github#2040`.
