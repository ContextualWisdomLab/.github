...
- Extended that guard after sweeping branch protection across all 76 organization repositories: 13 use classic protection, and several pin the *job names* the central workflows declare -- `opencode-review` and `coverage-evidence` are each required by 7 repositories, `strix` by 5, `scan-pr-queue` by 4, `required-workflow-bootstrap` by 3, and `coverage-source-tree` by 2. `strix` and `coverage-source-tree` are required by siblings but not by `.github`, so a sweep of this repository alone missed them. The check now resolves each job's *effective* check-run name (its `name:` when present, its job id only otherwise) rather than accepting either spelling, which previously let a renamed `name:` pass whenever the job id still matched.
