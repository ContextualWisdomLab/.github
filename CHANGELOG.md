...
- Publish exact-head review fixes, conflict repairs, and clean rebases through a
  fork-backed stacked pull request when and only when an all-branch ruleset
  rejects the normal push with the specific PR-required `GH013` diagnostic;
  preserve head guards, environment-only Git authentication, idempotent stack
  reuse, bounded post-push verification through GitHub PR API consistency lag,
  independent review and checks, and fail-closed handling for every unrelated
  error (`ContextualWisdomLab/.github#1199`).
