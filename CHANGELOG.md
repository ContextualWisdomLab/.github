...
- Preserved the OSV base-scan JSON in the runner-temporary mount before a cross-fork head checkout replaces the workspace, then restored the exact artifact for the required base/head vulnerability comparison instead of failing every fork PR with a missing `old-results.json`.
