...
- Retried transient GitHub visibility lookups in the required Strix job so a timeout, 408, 5xx, 429, empty/non-boolean response, leftover CONNECT `200`, or authenticated HTTP 403 rate-limit (installation budget / secondary rate limit) no longer aborts the scan before it starts. Rate-limit 403 uses a distinct 3-attempt budget and a 30–60s bounded wait (Retry-After / X-RateLimit-Reset when present, otherwise 30/60s). Multi-block `gh --include` output uses the terminal HTTP status, so a leading proxy `200 Connection established` cannot suppress a later 429/503 retry, a later timeout/reset marker, or a CONNECT-only drop. A real 401/403/404 on a missing or unauthorized repository stays fail-closed and is never treated as a source finding.
