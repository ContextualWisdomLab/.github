...
- Upgraded the required Strix scanner to 1.5.3 so a completed penetration-test report is written atomically and the process exits instead of hosting a local viewer, while keeping `cryptography==50.0.0` through a documented compile-time override and a `--require-hashes --no-deps` install of the complete lock. After install the workflow fail-closes unless `importlib.metadata` reports both pins. This closes the buyer-visible red Strix checks that discarded real findings after a 1.0.4 crash-after-print (ContextualWisdomLab/.github#952).
