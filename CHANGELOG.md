...
- Required every package line to carry a `--hash=` pin before pip-audit treats a requirements file as a complete hashed lock. A lone `--require-hashes` directive, a mixed hashed-plus-unhashed file, a filename-only wheel path, or a hash-shaped pip option no longer receives `--disable-pip`. Invalid UTF-8 and symlink/special-file inputs fail before any audit command, and workflow log paths are escaped.
