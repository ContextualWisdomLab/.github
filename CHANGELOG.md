...
- Grant the local agent-mention router the `actions: write` permission required for its repository-dispatch ledger, and remove the unsupported `concurrency.queue` key so the workflow passes actionlint and can enqueue exact-head review work.
