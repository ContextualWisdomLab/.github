...
- **Performance**: Replaced the redundant regular-expression compilation and
  `finditer` calls in `label_starts` with bounded literal `str.find` scanning,
  preserving non-overlapping label semantics while reducing per-label overhead.
