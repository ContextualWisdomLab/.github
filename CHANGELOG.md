### Proposed reusable Python source-tree test contract

- Adds a native-PR-only test workflow using the consumer's locked uv dependencies
  on an isolated hosted runner. It validates PR identity before checkout and
  runs the full source-tree test directory without an editable package build.
  Consumer adoption awaits independent review, protected publication and an
  immutable release. See [the decision](docs/adr/python-source-tests.md).
