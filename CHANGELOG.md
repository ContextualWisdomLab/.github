...
- Retried transient GitHub visibility lookups in the required Strix job so a timeout, 5xx, 429, empty/non-boolean response, or authenticated HTTP 403 rate-limit (installation budget / secondary rate limit) no longer aborts the scan before it starts. Rate-limit 403 uses a distinct 3-attempt budget and a 15–20s bounded wait (Retry-After / X-RateLimit-Reset when present, otherwise 15/20s). A real 401/403/404 on a missing or unauthorized repository stays fail-closed and is never treated as a source finding.
