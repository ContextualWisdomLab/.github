...
- Deferred a trusted base Python lock only when pip proves the package index answered with alternative versions for the same exact pin but no compatible distribution exists for the coverage interpreter/platform; registry outages, empty version sets, mismatched diagnostics, and unexplained resolver failures remain fatal.
