...
- Accept both current dot-region and legacy hyphen-region Figma S3 image hosts while retaining the Figma bucket prefix and `amazonaws.com` suffix allowlist, and track Figma's live unauthenticated `/v1/me` response change from HTTP 403 `Invalid token` to HTTP 401 `Missing credentials`.
