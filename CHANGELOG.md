...
- Added hash-pinned `pytest-asyncio==1.4.0` and `typing-extensions==4.16.0` plus an executed `scripts/ci/ensure_opencode_asyncio_toolchain.sh` import so the coverage sandbox can run marked asyncio tests without rewriting the hashed review-dispatch workflow or colliding with ContextualWisdomLab/.github#896 root ADRs.
