...
- Materialized base Python locks only when every package line is an exact SHA-256 pin or a bounded relative `-r`/`--requirement` include; direct `.txt` children of a `requirements` directory are now selected when hash-pinned, and accepted source-relative includes are rewritten to their deterministic generated lock names so flattened Docker build contexts remain pip-resolvable. A lone `--require-hashes` directive, a dotted include such as `./lock.txt`, `-r other-hashes.txt`, or an include whose exact base target was not materialized now fails closed instead of entering a broken trusted build context.
