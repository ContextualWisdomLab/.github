### OpenCode dispatch admits the ruleset-covered research repositories

- Every `late-life-anxiety-reanalysis` OpenCode dispatch failed in `validate-pr-metadata` with "repository_dispatch authorization rejected … absent from the configured exact repository allowlist" (for example run 35579928486, PR #243; 14 failed dispatches since 2026-09-19). Org ruleset 18156473 injects the required `opencode-review.yml` bootstrap there, and that bootstrap fails closed until the dispatch publishes an exact-head verdict, so no late-life PR (including #248) could ever get an OpenCode review. `korean-writing-skills` (PR #3) had the same gap. Both are added to `OPENCODE_REPOSITORY_DISPATCH_TARGETS` (live repository variable, the source of truth) and its mirror `scripts/ci/opencode_repository_dispatch_targets.json`. `test_ruleset_covered_research_repositories_are_dispatch_targets` pins them. Other covered repositories that are still absent (html4tree, argos, seedream_evasepic, orca and others seen in failed dispatches) are left as an owner decision, because admitting them adds review load while the Actions queue is saturated.
