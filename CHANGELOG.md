...
- Fail closed when GitHub dependency-review evidence is unavailable (HTTP 403/404, empty or malformed status, or transport failure, including curl exit 18 with a printed 200) instead of treating those outcomes as a clean skip; the probe records allowlisted repository visibility with the exact base and head SHAs and never prints the API body.
