### OpenCode dispatch admits targets dynamically instead of by a static repository list

- `opencode-review-dispatch.yml` `validate-pr-metadata` no longer gates targets on the hand-maintained `OPENCODE_REPOSITORY_DISPATCH_TARGETS` list. Org ruleset 18156473 requires the OpenCode bootstrap on every covered repository, and that bootstrap fails closed until the dispatch publishes an exact-head verdict. The two lists had drifted: every `late-life-anxiety-reanalysis` dispatch failed with "absent from the configured exact repository allowlist" (for example run 35579928486, PR #243; 14 since 2026-09-19), and html4tree, argos, seedream_evasepic and orca were rejected the same way. A `repository_dispatch` target is now admitted only when (1) it matches `ContextualWisdomLab/<name>`, (2) an OpenCode GitHub App installation token is available (the PAT entries of the `GH_TOKEN` fallback chain never authorize admission, and the live PR read in this step uses the App token), (3) the target appears in `/installation/repositories` for that token, which proves both existence and App installation, and (4) the existing live PR state, base and head comparison matches. External owners, a missing App token or installation, and stale heads fail closed. A new organization repository passes without any variable change.
- Compatibility period: the variable is still read as `LEGACY_DISPATCH_TARGETS`, and a target it would have rejected produces a `::notice::` only. Removal plan: drop that read and the variable from this workflow after two weeks without a legacy notice on an admitted run. The schedulers (`pr-review-merge-scheduler.yml`, `pr-review-fix-scheduler.yml`), `agent-mention-router.yml` and the hourly mirror (`scripts/ci/opencode_repository_dispatch_targets.json` with its drift test) still consume the list. They get the same dynamic check in follow-ups and are unchanged here.
- Tests: `test_opencode_dispatch_admits_new_org_repository_without_variable_change` and `test_opencode_dispatch_rejects_external_uninstalled_or_stale_targets` (7 cases) run the real step against a network-free `gh` stub. On the previous workflow 8 fail; with this change they pass. The independent review-dispatch blob pin moves to `8ee047b7`. Supersedes #2328, whose static-list extension was reverted on the live variable.
