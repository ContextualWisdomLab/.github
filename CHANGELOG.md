...
- Classify Strix `ModelBehaviorError` with `Vulnerabilities 0` as a
  backend-unavailable skip so the required check does not fail-closed on
  an LLM classifier flake. `Vulnerabilities [1-9]` still fail the check.
