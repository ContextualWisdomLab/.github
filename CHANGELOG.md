...
- Added hash-pinned `pytest-asyncio==1.4.0` and `typing-extensions==4.16.0`, execute `scripts/ci/ensure_opencode_asyncio_toolchain.sh` after the quality-job lock install, and collect a marked coroutine so the coverage sandbox can run buyer asyncio suites without rewriting the hashed review-dispatch workflow or colliding with ContextualWisdomLab/.github#896 root ADRs.
...
- Extended the hash-pinned Strix full-suite runner with `pytest-asyncio` and `typing-extensions`, matching the trusted OpenCode test toolchain so exact-head security checks collect asynchronous tests instead of failing on `ModuleNotFoundError`.
