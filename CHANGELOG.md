...
- Restore the trusted LLVM 19 Rust coverage producer pin in the coverage image, Docker sandbox env, and `ensure_rust_toolchain` guard, and fail closed when `REVIEW_DISPATCH_BLOB_SHA` no longer matches `git hash-object` of `.github/workflows/opencode-review-dispatch.yml` so a pull-request-head helper cannot silently change the coverage producer.
