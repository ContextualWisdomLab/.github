...
- PyO3 peer-evidence deferral no longer treats a prose file under `docs/requirements/` as a lock change; only `.in` / `.txt` / `.lock` names in a `requirements` path invalidate the source-only sandbox skip.
