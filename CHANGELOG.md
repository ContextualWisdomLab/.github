...
- Reject malformed dependency-review revisions and repository names, including `.`/`..` path segments, before the compare request so an empty SHA or `../` repository cannot mint a green `supported=true` from a forged HTTP 200.
