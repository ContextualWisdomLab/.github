...
12. the gate accepts only a required packet bound to the exact reviewed head and rejects a predecessor packet or missing packet; and
13. every code/test/requirements seam exercised by the permanent observed-probe workflow is included in that workflow's `pull_request.paths`, so lockfile-only or loader-only changes cannot bypass the focused contracts.
