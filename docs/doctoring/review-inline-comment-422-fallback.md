...
Manual-edit excerpt, without repeating the leftover reason bullet,
and deferred leftovers appear before leftovers that were already
posted. The first success uses `REQUEST_CHANGES` plus the review
...
- `tests/test_opencode_inline_comment_fallback.py` pins safe-pair extraction,
  ...
  without a duplicate reason bullet, and
