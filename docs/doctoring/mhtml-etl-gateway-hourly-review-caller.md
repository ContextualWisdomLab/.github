# mhtml-etl-gateway hourly review-repair caller
...
A paying buyer of governed archive ingest would feel live
mhtml-etl-gateway pull requests stalling while hourly NVIDIA NIM repair
scanned only Clearfolio, DiskSage, and fast-mlsirm. Live heads such as
ContextualWisdomLab/mhtml-etl-gateway#34 (HTML-table XSS extraction),
ContextualWisdomLab/mhtml-etl-gateway#36 (Postgres loader evaluation),
and ContextualWisdomLab/mhtml-etl-gateway#35 (normalize-text hot path)
target `main` and never enter those other callers.

The caller does not implement review or mutation logic itself.
mhtml-etl-gateway remains standalone; naruon consumes loaded assets
without owning the archive parser. Privileged automation stays in
...
This preserves an in-flight bounded RCA instead of discarding ingest
...
NVIDIA NIM work, plus XSS or loader analysis, can legitimately approach
...
`ContextualWisdomLab/mhtml-etl-gateway` target. Missing or mismatched
configuration fails before mutation credential materialization.

## Security, standalone operation, and modularity

The caller adds no mhtml-etl-gateway runtime dependency, database object,
network endpoint, tenant authority, or product credential.
mhtml-etl-gateway continues to run as a standalone ingest gateway.
Naruon and other CWL services may consume its tables, but they cannot
...
protected-main consumer run proving the exact mhtml-etl-gateway
repository and `main` base. Source checks alone are not protected-main operational acceptance.
Merge still requires zero unresolved valid findings and a
qualifying independent non-author approval.

Rollback removes the mhtml-etl-gateway caller, its focused test,
