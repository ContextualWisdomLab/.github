# pg-erd-cloud hourly review-repair caller
...
`ContextualWisdomLab/pg-erd-cloud` (cloud-erd.app — ERD workspace for
developers and data architects). The caller runs at minute 13, delegates
...
A paying buyer of the ERD workspace would feel live pg-erd-cloud pull
requests stalling while hourly NVIDIA NIM repair scanned only Clearfolio,
DiskSage, and fast-mlsirm. Live heads such as
ContextualWisdomLab/pg-erd-cloud#916 (control-character insertion),
#918 (schema-field validation), and #919 (handle-id generation) target
`main` and never enter those other callers.

The caller does not implement review or mutation logic itself.
pg-erd-cloud remains standalone; naruon does not own its schema canvas.
...
This preserves an in-flight bounded RCA instead of discarding ERD
...
NVIDIA NIM work, plus schema-canvas or control-character analysis, can
...
`ContextualWisdomLab/pg-erd-cloud` target. Missing or mismatched
configuration fails before mutation credential materialization.

## Security, standalone operation, and modularity

The caller adds no pg-erd-cloud runtime dependency, database object,
network endpoint, tenant authority, or product credential. pg-erd-cloud
continues to run as a standalone ERD workspace. Naruon and other CWL
services may consume exported diagrams, but they cannot weaken its
exact-head, approval, or security gates.

## Verification and rollback

Machine-checkable contracts require the exact target/base, minute 13
...
protected-main consumer run proving the exact pg-erd-cloud repository and
...
Rollback removes the pg-erd-cloud caller, its focused test, doctoring, and
