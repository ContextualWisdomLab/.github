# noema hourly review-repair caller
...
`ContextualWisdomLab/noema` (the shared agent runtime and lightweight
quarantine sandbox: Pydantic-AI / Codex-Python review agent in CI plus
the do-anything agent inside naruon). The caller runs at minute 19,
...
A paying buyer of the agent runtime would feel live noema pull requests
stalling while hourly NVIDIA NIM repair scanned only Clearfolio,
DiskSage, and fast-mlsirm. The live queue is drafts only: heads such as
ContextualWisdomLab/noema#404 (OIDC verification coverage),
ContextualWisdomLab/noema#403 (remaining LLM jobs through
contextual-orchestrator), and ContextualWisdomLab/noema#71 (workflow
source binding and MSA boundaries) target `main` and never enter those
other callers.

The caller does not implement review or mutation logic itself. Noema
remains standalone; naruon and wardnet import the runtime without owning
its quarantine sandbox. Privileged automation stays in
...
This preserves an in-flight bounded RCA instead of discarding sandbox or
review-agent evidence when the next hourly heartbeat arrives. The reusable
...
NVIDIA NIM work, plus noema quarantine and OIDC envelope analysis, can
...
`ContextualWisdomLab/noema` target. Missing or mismatched
configuration fails before mutation credential materialization.

## Security, standalone operation, and modularity

The caller adds no noema runtime dependency, database object, network
endpoint, tenant authority, or product credential. Noema continues to
run as a standalone agent runtime and quarantine sandbox. Naruon,
wardnet, and other CWL services may import noema, but they cannot weaken
its exact-head, approval, or security gates.

## Verification and rollback

Machine-checkable contracts require the exact target/base, minute 19
...
protected-main consumer run proving the exact noema repository and
...
Rollback removes the noema caller, its focused test, doctoring, and
