...
importing the orchestrator into this repository: a literal `127.0.0.1` or `::1`
endpoint may use HTTP only when its scheme, host, and port match the configured
`CONTEXTUAL_ORCHESTRATOR_BASE_URL` origin exactly and every resolver result is
loopback. Any other loopback literal or port must use HTTPS. Hostnames that
merely resolve to loopback do not receive this exception. Provider routing,
...
2. restricts plaintext HTTP to the exact configured loopback sidecar origin;
