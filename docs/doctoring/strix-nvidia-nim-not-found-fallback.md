...
The outer workflow may classify exhausted provider infrastructure as neutral when
the run log contains no accepted vulnerability signal, or when the trusted gate
has already printed that leftover console severity is incomplete log-only
evidence. A non-zero vulnerability count without that gate verdict remains
blocking. Scanner reports and attempt logs remain available as artifacts.
