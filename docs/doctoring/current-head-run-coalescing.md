...
The live-head admission and coalescing work share one job. Workflow-level concurrency includes repository, PR number, and event head SHA: duplicate events for one exact head collapse, while a delayed stale-head event cannot cancel the live-head cleanup. The first step re-fetches the PR and gates the remaining steps. This avoids the former two-job admission dependency, which could leave the cleanup worker waiting behind the queue pressure it was meant to relieve.
