# newsdom-api hourly review-repair caller
...
A paying buyer of the recognition sidecar would feel live newsdom-api
pull requests stalling while hourly NVIDIA NIM repair scanned only
Clearfolio, DiskSage, and fast-mlsirm. Live heads such as
ContextualWisdomLab/newsdom-api#634 (form-field length DoS),
ContextualWisdomLab/newsdom-api#632 (signal-killed PDF validators),
ContextualWisdomLab/newsdom-api#639 (page-filter CLI), and
ContextualWisdomLab/newsdom-api#638 (OpenAPI response examples) target
`develop` and never enter those other callers.

The caller does not implement review or mutation logic itself.
newsdom-api remains standalone; naruon consumes parsed DOM without owning
the PDF validator. Privileged automation stays in
...
This preserves an in-flight bounded RCA instead of discarding PDF/DOM
...
NVIDIA NIM work, plus PDF validator or form-bound analysis, can
...
`ContextualWisdomLab/newsdom-api` target. Missing or mismatched
configuration fails before mutation credential materialization.

## Security, standalone operation, and modularity

The caller adds no newsdom-api runtime dependency, database object,
network endpoint, tenant authority, or product credential. newsdom-api
continues to run as a standalone recognition sidecar. Naruon and other
CWL services may consume its DOM output, but they cannot weaken its
...
protected-develop consumer run proving the exact newsdom-api repository and
...
Rollback removes the newsdom-api caller, its focused test, doctoring, and
