...
1. PR validation proves manifest, mutation projection, concurrency refusal, redacted failures, post-write convergence, exact identity binding, trusted-main workflow gating, and 100% owned statement/branch/docstring coverage.
2. After source reaches protected `main`, provision the protected environment and dedicated credential, then enable the repository variable.
...
7. Keep the reconciler enabled for drift repair only after the first successful canary. If the API identity or editable schema changes, it fails closed and requires a reviewed source update.
