...
`ContextualWisdomLab/RankWeave` (standalone Apache-2.0 retrieval fusion,
evaluation, statistical comparison, and TREC benchmarking that uses only
the Python standard library). The caller runs at minute 18, delegates to
the product-neutral central review-fix scheduler, inspects at most 50
open pull requests targeting protected `main`, and dispatches at most
one bounded repair per heartbeat.

A paying buyer of governed retrieval fusion would feel live RankWeave
pull requests stalling while hourly NVIDIA NIM repair scanned only
Clearfolio, DiskSage, and fast-mlsirm. Live heads such as
ContextualWisdomLab/RankWeave#36 (restore executable hourly
governance), ContextualWisdomLab/RankWeave#37 (credential-first hourly
loop), and ContextualWisdomLab/RankWeave#38 (orphaned workflow
identities) target `main` and never enter those other callers.

The caller does not implement review or mutation logic itself.
RankWeave remains standalone; naruon and other CWL services may consume
fused rankings without owning the fusion engine. Privileged automation
...
This preserves an in-flight bounded RCA instead of discarding fusion,
TREC, or family-wise error evidence when the next hourly heartbeat
...
NVIDIA NIM work, plus ranking-evaluation RCA, can legitimately approach
...
`ContextualWisdomLab/RankWeave` target. Missing or mismatched
configuration fails before mutation credential materialization.

## Security, standalone operation, and modularity

The caller adds no RankWeave runtime dependency, database object,
network endpoint, tenant authority, or product credential. RankWeave
continues to run as a standalone retrieval-fusion module. Naruon and
other CWL services may consume its rankings, but they cannot weaken its
...
protected-main consumer run proving the exact RankWeave repository
and `main` base. Source checks alone are not protected-main operational
acceptance. Merge still requires zero unresolved valid findings and a
qualifying independent non-author approval.

Rollback removes the RankWeave caller, its focused test, doctoring, and
