...
`ContextualWisdomLab/appguardrail` (application security guardrails that
collect org security and CI failures plus Strix findings as issues). The
caller runs at minute 41, delegates to the product-neutral central
review-fix scheduler, inspects at most 50 open pull requests targeting
protected `develop`, and dispatches at most one bounded repair per
heartbeat.

A paying buyer of the security-guardrail plane would feel live
appguardrail pull requests stalling while hourly NVIDIA NIM repair
scanned only Clearfolio, DiskSage, and fast-mlsirm. Live heads such as
ContextualWisdomLab/appguardrail#964 (fail closed on ambiguous
severity), ContextualWisdomLab/appguardrail#967 (bind publication to
protected develop), ContextualWisdomLab/appguardrail#970 (retention
audit diligence), and ContextualWisdomLab/appguardrail#971 (mutable
MultipartFile byte-array exposure) target `develop` and never enter
those other callers.

The caller does not implement review or mutation logic itself.
Appguardrail remains standalone; naruon and wardnet consume its findings
but do not own the SAST runtime. Privileged automation stays in
...
This preserves an in-flight bounded RCA instead of discarding SAST
...
NVIDIA NIM work, plus detector or release-integrity analysis, can
...
continues to run as a standalone security-guardrail service. Naruon,
wardnet, and other CWL services may feed CI or Strix findings into it,
but they cannot weaken its exact-head, approval, or security gates.
...
and `develop` base. Source checks alone are not protected-develop operational acceptance.
Merge still requires zero unresolved valid findings and a
qualifying independent non-author approval.

Rollback removes the appguardrail caller, its focused test, doctoring, and
