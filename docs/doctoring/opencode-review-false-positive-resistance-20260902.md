...
For the **gated** CI/runtime surfaces, uncertainty is now represented only with states the gate can carry. If a heuristic candidate remains unproven after tracing, it is left uncounted rather than fabricated into a blocker; if the rest of the review independently satisfies `APPROVE`, bounded uncertainty belongs in `adversarial_validation.residual_risk`. `REQUEST_CHANGES` is used only when current-head source or bounded evidence confirms an explicit authorization, verification, or required-evidence contract is missing or violated, so the existing confirmed-probe/finding schema remains valid. The standalone `code-reviewer-prompt.md` may still surface an uncounted candidate as `NEEDS_INFO`, because that human-facing contract explicitly supports that verdict.
...
The regression is paragraph-scoped so scattered substrings cannot satisfy the contract. The runtime template also retains current-head and language-evidence authority, and the CI prompt retains its established adversarial probe-count thresholds. A dedicated gated-surface regression now rejects instructions that try to encode uncertainty as an unsupported `NEEDS_INFO` result and requires bounded uncertainty to remain schema-representable through `residual_risk` or a separately confirmed review-contract violation.
...
The schema-representability finding was not left as a merged-PR comment. After #1654 landed on protected `main`, a fresh owner-side branch was cut from the exact merge commit, a RED regression was committed first, and the gated CI/runtime prompts were repaired so their uncertainty semantics agree with the two-result control schema. This explicitly exercises the requested cross-file/state-machine false-negative class: prompt policy, normalizer result enum, and publication-gate semantics must agree.
