...
The same `fstat()` call that proves regular-file type now captures `(st_dev, st_ino)` for each open descriptor. Manifest and predicate snapshots are refused before semantic validation when those identities match. This avoids adding a separate pathname `stat()` race and does not require `st_nlink == 1`; unrelated hard links are allowed unless the two receipt roles themselves resolve to the same inode.
...
Manifest and predicate pathnames must be distinct, and their opened descriptor identities must also be distinct. Missing leaves, symlinks, FIFOs/directories, symlinked parents, oversize files, same-inode hard-link aliases, and semantically invalid bounded pairs fail closed. `O_NONBLOCK` prevents a FIFO from stalling the signer before the regular-file test executes.
...
The owner quality gate covers the reader and its hostile-file contracts at 100% owned line/branch/docstring coverage. The tests exercise valid exact bytes, per-file size refusal before semantic parsing, missing/symlink/non-regular/FIFO/ancestor-symlink objects, identical paths, same-inode hard-link aliases before semantic validation, semantic failure after bounded intake, stable CLI exit behavior, executable dispatch, and immutable-sibling bootstrap refusal.
