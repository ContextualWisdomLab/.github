...
The newest cohort has explicit source ownership: `PolicyWeave#1` carries its exact-cased badge and `docs/index.md`; `supply-chain-control-plane#1` now carries its exact badge and bounded Pages landing source on the active product writer; `learning-management-platform#1` already owns both the product-first README badge and `docs/index.md`; `learning-content-studio#1` owns the badge while stacked `learning-content-studio#8` owns only `docs/index.md`; and `learning-record-store#1` owns its exact badge while stacked `learning-record-store#7` owns only the bounded Pages landing source. Their live repositories still report Pages disabled until protected integration and trusted reconciliation complete.
