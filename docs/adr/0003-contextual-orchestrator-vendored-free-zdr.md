...
- Decision: Route every central CI review write/model execution that touches contracts in this repository through the **vendored** `contextual-orchestrator` gateway, served as a per-runner sidecar. OpenCode, Noema, and (as of the 2026-08-30 amendment) Strix all use the fail-closed zero-cost virtual model id `orchestrator/free`, with **Zero Data Retention (ZDR)-compliant routes required** inside that pool.
...
3. **ZDR-first within each cost tier**: `scripts/ci/zdr_policy.py` defines ZDR
   ...
   fetched when egress allows it. Only canonical `openrouter/<model>` feed keys
   are accepted. OpenRouter routes require exact feed membership; for every
   other discovered provider, that canonical feed's model identity is used as
   selection evidence for a matching candidate row. Noncanonical, nonmatching,
   or ambiguous candidates remain non-ZDR. Otherwise the dated static
   attestation table is used, never a fabricated policy.
   Cost-tier ordering always ranks free routes ahead of priced routes: a
   discovered but non-ZDR free route therefore cannot suppress an attested
   priced route from the `auto` pool; when an admitted free tier exists it
   remains the exclusive primary, and the admitted priced tier remains
   fallback-only. `scripts/ci/contextual_orchestrator_review_policy.py` turns
   the discovery report into a free-first, cost-evidence-ranked,
   ZDR-prioritized, provider-family-diverse agents catalog (primary/secondary
   NVIDIA keys share one outage-domain family), capped in size, in the
   orchestrator's own `ModelAgent` schema. OpenRouter itself is never a
   routable candidate in either pool; it is consumed only as ZDR evidence.
4. **Wiring**: `pr-review-autofix.yml` and the Required OpenCode dispatch
   ...
   `noema-review.yml`, `strix.yml` (as of the 2026-08-30 amendment below), and
   the Required OpenCode dispatch provision the same sidecar and use the
   loopback chat-completions/API-compatible URL with virtual model
   `orchestrator/free`; every target passes the explicit ZDR-required policy
   to the gateway, while visibility remains independently validated as trust
   metadata. The sidecar also fails closed when the ZDR requirement is missing
   or false. Provider diversity and cost-evidence classification for any
   caller that opts into the separately governed `orchestrator/auto` pool
   remain delegated to the gateway rather than embedding a second routing
   policy in GitHub Actions. Strix has no external fallback. Noema reviewer
   identity remains `NOEMA_REVIEW_TOKEN` / GitHub App / OIDC and is still
   never `github.token`; Autofix mutation still requires
...
- The autofix/OpenCode review paths no longer hard-code any provider base URL
  ...
  discovery + cost routing, under the zero-cost pool, with ZDR routes
  required. Absent price metadata is treated as neither free nor paid-route
  evidence, remaining auditable but ineligible.
- The separately governed `orchestrator/auto` pool (free-first, then fully
  price-attested priced routes; unknown-cost routes remain auditable but
  ineligible) remains available in
  `contextual_orchestrator_review_policy.py`/the sidecar for any caller that
  opts into it explicitly, with the same fail-closed ZDR admission as the
  free pool. Strix itself defaults to the zero-cost `orchestrator/free` pool
  as of the 2026-08-30 amendment below, not `orchestrator/auto`.
