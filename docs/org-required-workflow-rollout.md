...
- Model token posture: use the organization `NVIDIA_NIM_API_KEY` secret only. Workflows bind it to process env `NVIDIA_API_KEY`. GitHub Models is not used; if the NIM secret is unset, OpenCode and Strix fail closed instead of falling through to another provider.
