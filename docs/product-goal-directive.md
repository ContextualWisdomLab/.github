...
**Note (flagged by CodeRabbit on this PR, 2026-08-30; pool description updated 2026-09-02 — see the correction below):** section 8's quoted text describes `contextual-orchestrator`'s general product capability — broad model/modality support and all-five-secret auto model discovery as a *design principle for the orchestrator itself*. It does not specify, and must not be read as overriding, which pool each CI consumer routes through: that is governed exclusively by `docs/adr/0003-contextual-orchestrator-vendored-free-zdr.md` and its doctoring records. As of the pin flip recorded below, `OpenCode`, `Noema`, **and** `Strix` all use the fail-closed, ZDR-prioritized `orchestrator/free` pool — the original 2026-08-30 wording of this note ("only `Strix` security analysis uses the provider-diverse `orchestrator/auto` pool") described the pre-pin-flip state and is historical, not current; private/internal review targets require an attested ZDR-only catalog and never fall back to a non-ZDR provider. Do not loosen any CI consumer's pool or credential scope on the strength of this section's general wording alone.
...
**Resolution (2026-09-02):** the "open, unreviewed risk" this note describes is closed as of today. In a session auditing whether OpenCode/Strix gateway enforcement is real (`docs/doctoring/contextual-orchestrator-gateway-enforcement-audit-20260902.md`), the owner reviewed ADR-0003's 2026-08-31 correction directly and gave an explicit, current confirmation — recorded verbatim in ADR-0003's 2026-09-02 amendment — that `orchestrator/free` is the intended pin for both OpenCode and Strix across all GitHub Actions workflow usage. This does not retroactively make the original 2026-08-30 amendment's fabricated "the org owner explicitly directed this" attribution true — that remains false as history, exactly as the 2026-08-31 correction states — it is a new, real decision that happens to land on the same configuration. The residual single-outage-domain availability risk this note and ADR-0003 both describe (Strix has no external fallback under `orchestrator/free`) is unchanged and is now a knowingly accepted trade-off rather than an unreviewed one; `free_account_diversity` remains the live monitoring evidence for it.
