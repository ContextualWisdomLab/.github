...
**Note (2026-08-30, same-date follow-up instruction):** a later same-date loop instance explicitly asked that Strix also route through `orchestrator/free`. Per this file's own conflict policy, that was not applied by editing the pin directly: `ContextualWisdomLab/.github#1433` added `free_family_diversity` evidence to `scripts/ci/contextual_orchestrator_review_policy.py` (see ADR-0003's 2026-08-30 addendum and `docs/doctoring/contextual-orchestrator-strix-free-diversity-evidence.md`) rather than flipping the pin on the strength of the instruction alone. A first attempt to wire that evidence into `strix.yml` unconditionally (`#1437`, first draft) was rejected on exact-head review for reintroducing the exact single-outage-domain regression this section's pool split exists to prevent.

**Follow-up note (2026-08-30, evidence-gated wiring landed):** `#1437` was corrected to wire `strix.yml`'s model-resolution step to the `free_family_diversity` evidence: it selects `orchestrator/free` only when that count is `>= 2`, and falls back to `orchestrator/auto` — the pool this section's Note above already authorizes for Strix — otherwise. See [`docs/adr/0020-strix-orchestrator-free-pool.md`](adr/0020-strix-orchestrator-free-pool.md) (refining, not superseding, `docs/adr/0003-contextual-orchestrator-vendored-free-zdr.md`'s Strix-specific wiring bullet, whose original decision text remains as history with an Amendment section) for the decision, the negative-fixture guarantee that a diversity of 0 or 1 cannot weaken the resolved model to `orchestrator/free`, and the residual risk (live-market free-tier availability; a separate, independently tracked request-time-failover gap in `contextual-orchestrator`) it documents rather than hides. The rest of the Note above — that pool/credential-scope routing is governed exclusively by these ADRs, not by section 8's general wording, and that private/internal targets require an attested ZDR-only catalog — still holds. Do not re-loosen any CI consumer's pool or credential scope beyond what ADR-0003 and ADR-0020 currently authorize.
