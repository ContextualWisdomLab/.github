...
Every production component that calls contextual-orchestrator must follow [`docs/standards/contextual-orchestrator-consumer-contract.md`](docs/standards/contextual-orchestrator-consumer-contract.md): ordinary traffic uses adaptive `auto`, quality/capability is optimized before trustworthy known cost, unpriced models are not free, and provider-native structured-output passthrough must not silently pin an adaptive product path to one worker.
