...
Pending and dismissed reviews do not dispatch mention agents. Submitted review bodies react through GraphQL `addReaction`. GraphQL already-reacted eyes on a review body are success. The local mention job grants `reactions: write` so the optional eyes reaction is not a 403. See [`ARCHITECTURE.md`](ARCHITECTURE.md) and [`docs/doctoring/review-agent-mention-surfaces.md`](docs/doctoring/review-agent-mention-surfaces.md).
