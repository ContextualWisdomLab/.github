...
Private repositories use anonymous `opencode-free/*` reviewers only from an unchanged trusted-base policy; missing visibility and missing base SHA strip those aliases. The provider guard never forwards `COPILOT_GITHUB_TOKEN`. See [`docs/doctoring/opencode-private-free-model-policy.md`](docs/doctoring/opencode-private-free-model-policy.md).
