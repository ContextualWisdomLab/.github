...
Overview receipts treat only a closed ```suggestion fence on a non-LEFT comment as applyable. A leftover ```diff whose prose mentions the token, and every LEFT suggestion, stay leftover (`cannot-provide` / `LEFT`). Leftover receipt path/reason rows strip HTML comment delimiters and metacharacters before they enter `<!-- opencode-review-overview -->`. See [`ARCHITECTURE.md`](ARCHITECTURE.md) and [`docs/doctoring/review-inline-comment-422-fallback.md`](docs/doctoring/review-inline-comment-422-fallback.md).
