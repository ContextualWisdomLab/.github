...
Strix leftover-TUI neutralization honors `incomplete-evidence` and `failed after provider infrastructure or failure-signal output`. Materialize accepts only exact SHA-256 pins or a bounded relative `-r` include (no `.`/`..`); a lone `--require-hashes` directive is not trust evidence. See [`ARCHITECTURE.md`](ARCHITECTURE.md) and [`docs/doctoring/strix-incomplete-retry-severity.md`](docs/doctoring/strix-incomplete-retry-severity.md).
