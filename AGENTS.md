...
OpenCode APPROVE names a current-head file only as a whole path token (`example.py.bak` does not dispose `example.py`). See [`ARCHITECTURE.md`](ARCHITECTURE.md) and [`docs/doctoring/review-contract-per-file-disposition.md`](docs/doctoring/review-contract-per-file-disposition.md).
