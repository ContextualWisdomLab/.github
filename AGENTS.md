...
Wardnet's AI-SOC hourly caller scans `ContextualWisdomLab/wardnet` `main` via the product-neutral NVIDIA NIM scheduler (`NVIDIA_NIM_API_KEY` stays on the worker; never `COPILOT_GITHUB_TOKEN`). See [`docs/doctoring/wardnet-hourly-review-caller.md`](docs/doctoring/wardnet-hourly-review-caller.md).
