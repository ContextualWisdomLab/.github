...
Strix provenance skips a `run.json` that has no head/commit SHA metadata instead of substituting the scan-start SHA.
Materialize accepts only exact SHA-256 pins or a bounded relative `-r` include (no `.`/`..`).
