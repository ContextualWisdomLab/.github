...
Materialize accepts only exact SHA-256 pins or a bounded relative `-r` include (no `.`/`..`); a lone `--require-hashes` directive is not trust evidence. See [`docs/doctoring/strix-google-api-core-pin.md`](docs/doctoring/strix-google-api-core-pin.md).
