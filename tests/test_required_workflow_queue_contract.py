...
def test_strix_provider_outage_without_findings_remains_fail_closed() -> None:
    workflow = workflow_text("strix.yml")
    ...
    assert "Nvidia_nimException" in workflow
    assert "STRIX_FAIL_ON_MIN_SEVERITY: MEDIUM" in workflow
    assert "Strix did not produce complete passing security evidence" in workflow
