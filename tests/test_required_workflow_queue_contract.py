...
def test_sbom_generation_cancels_superseded_branch_scans() -> None:
    """A newer SBOM run for the same ref cancels the older run.

    SBOM generation re-reads the same release tag or branch ref, so a
    superseded run waiting behind the runner ceiling only delays its
    replacement. Issue #1988 found this ``true`` contracted by nothing;
    flipping it to ``false`` would silently let stale generations pile up.
    """
