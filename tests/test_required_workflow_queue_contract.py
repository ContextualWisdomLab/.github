...
def test_strix_provider_outage_without_findings_fails_closed() -> None:
    ...
    assert "Strix evidence incomplete" in run_step
    assert 'exit "$strix_rc"' in run_step
    assert "neutral skip" not in run_step
    assert "backend_unavailable_signal" not in run_step
    assert "reported_vulnerability_signal" not in run_step
