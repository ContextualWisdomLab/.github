"""Contract tests for pg-erd-cloud's bounded hourly review-repair caller."""

from pathlib import Path


CALLER = Path(".github/workflows/pg-erd-cloud-hourly-review-repair.yml")
DOCTORING = Path("docs/doctoring/pg-erd-cloud-hourly-review-caller.md")
...
def test_pg_erd_cloud_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """pg-erd-cloud receives one realistic ERD repair without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "13 * * * *"' in caller
    assert "group: pg-erd-cloud-hourly-review-repair" in caller
    assert "cancel-in-progress: false" in caller
    assert "uses: ./.github/workflows/pr-review-fix-scheduler.yml" in caller
    assert "target_repository: ContextualWisdomLab/pg-erd-cloud" in caller
...
def test_pg_erd_cloud_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_pg_erd_cloud_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/pg-erd-cloud" not in _read(SCHEDULER)


def test_pg_erd_cloud_doctoring_records_erd_activation_and_credentials() -> None:
    """Operators retain target-allowlist, ERD, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/pg-erd-cloud#916",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    quality = (
        ...
        "  pull_request:\n"
        "    paths:\n"
        "      - .github/workflows/pg-erd-cloud-hourly-review-repair.yml\n"
        "  push:\n"
        "    paths:\n"
        "      - docs/doctoring/pg-erd-cloud-hourly-review-caller.md\n"
        "          python -m compileall -q \\\n"
        "            tests/test_pg_erd_cloud_hourly_review_caller.py\n"
        "          git diff --check\n"
    )
    ...
    assert pull_request_paths == {".github/workflows/pg-erd-cloud-hourly-review-repair.yml"}
    assert push_paths == {"docs/doctoring/pg-erd-cloud-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_pg_erd_cloud_hourly_review_caller.py"}
    assert "docs/doctoring/pg-erd-cloud-hourly-review-caller.md" not in pull_request_paths
    assert ".github/workflows/pg-erd-cloud-hourly-review-repair.yml" not in compileall_paths


def test_focused_quality_workflow_tracks_pg_erd_cloud_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/pg-erd-cloud-hourly-review-repair.yml"
    doctoring = "docs/doctoring/pg-erd-cloud-hourly-review-caller.md"
    contract = "tests/test_pg_erd_cloud_hourly_review_caller.py"
