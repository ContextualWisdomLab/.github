...
def test_kill_process_group_cleans_descendants_and_handles_disappearing_groups(
    monkeypatch,
) -> None:
    """Cleanup signals a group after leader exit and tolerates a vanished group."""
    ...
    assert calls == [(10, bounded.signal.SIGKILL)]
...
def test_two_overflow_callbacks_share_one_limit_kill_before_final_cleanup(
    monkeypatch,
    tmp_path: Path,
) -> None:
    """Limit callbacks share one kill before the mandatory descendant cleanup."""
...
def test_run_joins_both_stream_captures_when_one_join_fails(
    ...
    monkeypatch.setattr(bounded, "kill_process_group", lambda _process: None)
