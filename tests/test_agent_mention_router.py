...
def test_graphql_eyes_reaction_treats_already_reacted_as_success() -> None:
        ...
        def request(self, args, *, input_payload=None):
            ...
            if args and args[0] == "graphql":
                return {
                    ...
                    "errors": [
                        {
                            "message": "Reaction already exists. You can only "
                            "react once with this content."
                        }
                    ],
                }
            return None
    ...
    class EmptyGraphqlClient(FakeClient):
        """Return an empty GraphQL object so success cannot be inferred."""

        def request(self, args, *, input_payload=None):
            """Serve the review lookup, then an empty GraphQL body."""
