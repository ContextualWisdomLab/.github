"""Contract tests for Wardnet's bounded hourly review-repair caller."""

from pathlib import Path


CALLER = Path(".github/workflows/wardnet-hourly-review-repair.yml")
...
def test_wardnet_caller_is_hourly_bounded_and_non_cancelling() -> None:
    ...
    assert "target_repository: ContextualWisdomLab/wardnet" in caller
...
def test_wardnet_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/wardnet" not in _read(SCHEDULER)


def test_wardnet_doctoring_records_soc_activation_and_credentials() -> None:
    """Operators retain target-allowlist, SOC, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/wardnet#76",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    assert pull_request_paths == {".github/workflows/wardnet-hourly-review-repair.yml"}
    assert push_paths == {"docs/doctoring/wardnet-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_wardnet_hourly_review_caller.py"}
    assert "docs/doctoring/wardnet-hourly-review-caller.md" not in pull_request_paths
    assert ".github/workflows/wardnet-hourly-review-repair.yml" not in compileall_paths


def test_focused_quality_workflow_tracks_wardnet_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/wardnet-hourly-review-repair.yml"
    doctoring = "docs/doctoring/wardnet-hourly-review-caller.md"
    contract = "tests/test_wardnet_hourly_review_caller.py"
