...
def test_rankweave_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """RankWeave receives one realistic retrieval-fusion repair without cancellation."""
    caller = _read(CALLER)
...
def test_rankweave_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_rankweave_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/RankWeave" not in _read(SCHEDULER)


def test_rankweave_doctoring_records_activation_and_credentials() -> None:
    """Operators retain target-allowlist and approval prerequisites."""
    ...
    for phrase in (
        ...
        "protected-main operational",
        "APA 7th references",
        "ContextualWisdomLab/RankWeave#36",
        "ContextualWisdomLab/RankWeave#37",
        "ContextualWisdomLab/RankWeave#38",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    assert push_paths == {"docs/doctoring/rankweave-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_rankweave_hourly_review_caller.py"}
    assert "docs/doctoring/rankweave-hourly-review-caller.md" not in pull_request_paths
    assert (
        ".github/workflows/rankweave-hourly-review-repair.yml"
...
def test_focused_quality_workflow_tracks_rankweave_contracts() -> None:
    ...
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/rankweave-hourly-review-repair.yml"
    doctoring = "docs/doctoring/rankweave-hourly-review-caller.md"
    contract = "tests/test_rankweave_hourly_review_caller.py"
