...
def _compileall_block(quality: str) -> str:
    """Return the compileall argument list before the whitespace gate."""
...
def test_inkspan_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """Inkspan receives one bounded editor repair without cancellation."""
    caller = _read(CALLER)
...
def test_inkspan_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_inkspan_doctoring_records_product_and_authority_boundaries() -> None:
    """Operators retain product evidence, authority, and acceptance gates."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/inkspan#373",
        "ContextualWisdomLab/inkspan#372",
...
def test_path_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    """A path in only one quality block cannot satisfy the other blocks."""
    quality = (
        "on:\n"
        "  pull_request:\n"
        "    paths:\n"
    ...
    assert pull_request_paths == {".github/workflows/inkspan-hourly-review-repair.yml"}
    assert push_paths == {"docs/doctoring/inkspan-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_inkspan_hourly_review_caller.py"}
...
def test_focused_quality_workflow_tracks_inkspan_contracts() -> None:
    ...
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/inkspan-hourly-review-repair.yml"
    doctoring = "docs/doctoring/inkspan-hourly-review-caller.md"
    contract = "tests/test_inkspan_hourly_review_caller.py"
