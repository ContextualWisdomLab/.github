"""Contract tests for pg-llm-batch's bounded hourly review-repair caller."""

from pathlib import Path


CALLER = Path(".github/workflows/pg-llm-batch-hourly-review-repair.yml")
DOCTORING = Path("docs/doctoring/pg-llm-batch-hourly-review-caller.md")
...
def test_pg_llm_batch_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """pg-llm-batch receives one batch-engine repair without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "1 * * * *"' in caller
    assert "group: pg-llm-batch-hourly-review-repair" in caller
    assert "cancel-in-progress: false" in caller
    assert "uses: ./.github/workflows/pr-review-fix-scheduler.yml" in caller
    assert "target_repository: ContextualWisdomLab/pg-llm-batch" in caller
...
def test_pg_llm_batch_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_pg_llm_batch_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/pg-llm-batch" not in _read(SCHEDULER)


def test_pg_llm_batch_doctoring_records_batch_activation_and_credentials() -> None:
    """Operators retain target-allowlist, batch engine, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/pg-llm-batch#227",
        "ContextualWisdomLab/pg-llm-batch#222",
        "ContextualWisdomLab/pg-llm-batch#221",
        "ContextualWisdomLab/pg-llm-batch#194",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    assert push_paths == {"docs/doctoring/pg-llm-batch-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_pg_llm_batch_hourly_review_caller.py"}
    assert "docs/doctoring/pg-llm-batch-hourly-review-caller.md" not in pull_request_paths
    assert (
        ".github/workflows/pg-llm-batch-hourly-review-repair.yml"
...
def test_focused_quality_workflow_tracks_pg_llm_batch_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/pg-llm-batch-hourly-review-repair.yml"
    doctoring = "docs/doctoring/pg-llm-batch-hourly-review-caller.md"
    contract = "tests/test_pg_llm_batch_hourly_review_caller.py"
