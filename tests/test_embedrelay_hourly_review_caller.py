"""Contract tests for EmbedRelay's bounded hourly review-repair caller."""
...
def test_embedrelay_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """EmbedRelay receives one realistic embedding-swap repair without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "20 * * * *"' in caller
    assert "group: embedrelay-hourly-review-repair" in caller
    assert "cancel-in-progress: false" in caller
    assert "uses: ./.github/workflows/pr-review-fix-scheduler.yml" in caller
    assert "target_repository: ContextualWisdomLab/EmbedRelay" in caller
...
def test_embedrelay_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_embedrelay_doctoring_records_embedding_activation_and_credentials() -> None:
    """Operators retain target-allowlist, embedding swap, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/EmbedRelay#3",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    assert pull_request_paths == {".github/workflows/embedrelay-hourly-review-repair.yml"}
    assert push_paths == {"docs/doctoring/embedrelay-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_embedrelay_hourly_review_caller.py"}
    assert "docs/doctoring/embedrelay-hourly-review-caller.md" not in pull_request_paths
    assert ".github/workflows/embedrelay-hourly-review-repair.yml" not in compileall_paths


def test_focused_quality_workflow_tracks_embedrelay_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/embedrelay-hourly-review-repair.yml"
    doctoring = "docs/doctoring/embedrelay-hourly-review-caller.md"
    contract = "tests/test_embedrelay_hourly_review_caller.py"
