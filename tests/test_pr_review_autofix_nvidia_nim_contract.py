...
REVIEW_DISPATCH_BLOB_SHA = "275aad4b82d39e7ede20ccc30d72cd32fbf4cbd2"
...
def test_independent_review_agent_key_system_is_unchanged() -> None:
    ...
    The workflow now legitimately changes to start an inference-only gateway.
    A whole-file hash would therefore reject the requested integration and
    would not identify which reviewer credential boundary changed. Assert the
    durable token expressions and verify that the model-execution step receives
    provider keys only; publication and scheduler steps retain their existing
    review-write sources.
    """
    ...
    assert "BYTEZ_API_KEY" in model_step
    assert "NVIDIA_NIM_API_KEY" in model_step
    assert "NVIDIA_NIM_API_KEY_SUB" in model_step
    assert "OPENROUTER_API_KEY" in model_step
    assert "OPENAI_API_KEY" in model_step
