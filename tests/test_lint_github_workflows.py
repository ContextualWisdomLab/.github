...
def test_linter_rejects_queue_max_without_literal_false(
    tmp_path: Path,
    cancel_value: str,
) -> None:
    """Dynamic or non-boolean cancellation cannot prove a lossless max queue."""

    environment, capture_dir = _tool_environment(tmp_path)
    workflow = tmp_path / "ambiguous-cancellation.yml"
    workflow.write_text(
        f"""name: ambiguous-cancellation
on: push
concurrency:
  group: exact
  queue: max
  cancel-in-progress: {cancel_value}
jobs: {{}}
""",
        encoding="utf-8",
    )
    ...
    assert result.returncode == 2
    assert "requires cancel-in-progress to be literal false or absent" in result.stderr
    assert not (capture_dir / "actionlint.json").exists()
