...
def test_workflow_provisions_sandbox_tool_and_reviewer_agent():
    ...
    assert "--leftover-diff-locations" in workflow
    helper = Path("scripts/ci/opencode_inline_comment_fallback.py").read_text(
        encoding="utf-8"
    )
    ...
    assert "leftover_diff_fence_receipts(payload)" in helper
    ...
    assert "seen_deferred" in helper
    assert "allowed=allowed" in helper
    assert "--leftover-diff-locations" in helper
    assert "--deferred-locations" in helper
    ...
    assert (
        "These comments still have a suggested-diff fence that GitHub cannot apply:"
        in Path("scripts/ci/opencode_inline_comment_fallback.py").read_text(encoding="utf-8")
    )
