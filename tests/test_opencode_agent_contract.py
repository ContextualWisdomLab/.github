...
def test_workflow_provisions_sandbox_tool_and_reviewer_agent():
    ...
    assert re.search(r"Run OpenCode PR Review model pool[\s\S]{0,240}timeout-minutes: 360", workflow)
    ...
    assert 'OPENCODE_MODEL_ATTEMPTS: "5"' in workflow
    assert 'OPENCODE_RUN_TIMEOUT_SECONDS: "900"' in workflow
    assert 'OPENCODE_EXPORT_TIMEOUT_SECONDS: "120"' in workflow
    assert 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "14400"' in workflow
    assert 'OPENCODE_BACKOFF_MAX_SECONDS: "600"' in workflow
