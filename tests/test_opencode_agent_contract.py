...
def test_workflow_provisions_sandbox_tool_and_reviewer_agent():
    ...
    assert re.search(r"Run OpenCode PR Review model pool[\s\S]{0,240}timeout-minutes: 50", workflow)
    assert 'OPENCODE_SMALL_CHANGE_TOTAL_BUDGET_SECONDS: "1500"' in workflow
    assert 'OPENCODE_MEDIUM_CHANGE_TOTAL_BUDGET_SECONDS: "2400"' in workflow
    assert 'OPENCODE_LARGE_CHANGE_TOTAL_BUDGET_SECONDS: "2700"' in workflow
    assert 'OPENCODE_UNKNOWN_CHANGE_TOTAL_BUDGET_SECONDS: "2400"' in workflow
    assert 'OPENCODE_RUN_TIMEOUT_SECONDS: "300"' in workflow
    assert 'OPENCODE_TOTAL_RETRY_BUDGET_SECONDS: "600"' in workflow
    assert 'OPENCODE_POOL_STEP_TIMEOUT_SECONDS: "2880"' in workflow
    assert 'timeout --kill-after=30s "${OPENCODE_POOL_STEP_TIMEOUT_SECONDS:-2880}s"' in workflow
    ...
    assert 'OPENCODE_SMALL_CHANGE_TOTAL_BUDGET_SECONDS: "1500"' in workflow
    assert 'OPENCODE_MEDIUM_CHANGE_RUN_TIMEOUT_SECONDS: "1500"' in workflow
    assert 'OPENCODE_MEDIUM_CHANGE_TOTAL_BUDGET_SECONDS: "2400"' in workflow
    assert 'OPENCODE_LARGE_CHANGE_RUN_TIMEOUT_SECONDS: "1800"' in workflow
    assert 'OPENCODE_LARGE_CHANGE_TOTAL_BUDGET_SECONDS: "2700"' in workflow
    assert 'OPENCODE_UNKNOWN_CHANGE_RUN_TIMEOUT_SECONDS: "1500"' in workflow
    assert 'OPENCODE_UNKNOWN_CHANGE_TOTAL_BUDGET_SECONDS: "2400"' in workflow
