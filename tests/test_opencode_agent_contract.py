...
def test_merge_scheduler_uses_escalating_mutation_credentials():
    ...
    # One scheduler job carries the workflow-token evidence. The branch pinned
    # two because it also set it on org-queue-sweep, a job main has since removed.
    assert workflow.count("SCHEDULER_WORKFLOW_TOKEN: ${{ github.token }}") == 1
