...
def test_naruon_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """naruon receives one realistic platform repair opportunity without cancellation."""
    caller = _read(CALLER)
...
def test_naruon_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_naruon_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/naruon" not in _read(SCHEDULER)


def test_naruon_doctoring_records_platform_activation_and_credentials() -> None:
    """Operators retain target-allowlist, develop-base, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/naruon#1388",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    assert pull_request_paths == {".github/workflows/naruon-hourly-review-repair.yml"}
    assert push_paths == {"docs/doctoring/naruon-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_naruon_hourly_review_caller.py"}
    assert "docs/doctoring/naruon-hourly-review-caller.md" not in pull_request_paths
    assert ".github/workflows/naruon-hourly-review-repair.yml" not in compileall_paths


def test_focused_quality_workflow_tracks_naruon_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/naruon-hourly-review-repair.yml"
    doctoring = "docs/doctoring/naruon-hourly-review-caller.md"
    contract = "tests/test_naruon_hourly_review_caller.py"
