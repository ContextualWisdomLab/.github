...
def test_naruon_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """naruon receives one bounded repair opportunity without cancellation."""
    caller = _read(CALLER)
...
def test_naruon_caller_preserves_read_only_explicit_secret_scope() -> None:
...
def test_naruon_doctoring_records_fail_closed_activation() -> None:
...
def test_focused_quality_workflow_tracks_naruon_contracts() -> None:
    ...
    assert quality.count(".github/workflows/naruon-hourly-review-repair.yml") == 2
    assert quality.count("docs/doctoring/naruon-hourly-review-caller.md") == 2
