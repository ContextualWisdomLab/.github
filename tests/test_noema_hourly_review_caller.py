"""Contract tests for noema's bounded hourly review-repair caller."""

from pathlib import Path


CALLER = Path(".github/workflows/noema-hourly-review-repair.yml")
DOCTORING = Path("docs/doctoring/noema-hourly-review-caller.md")
...
def test_noema_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """Noema receives one realistic sandbox repair opportunity without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "19 * * * *"' in caller
    assert "group: noema-hourly-review-repair" in caller
    assert "cancel-in-progress: false" in caller
    assert "uses: ./.github/workflows/pr-review-fix-scheduler.yml" in caller
    assert "target_repository: ContextualWisdomLab/noema" in caller
...
def test_noema_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_noema_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/noema" not in _read(SCHEDULER)


def test_noema_doctoring_records_sandbox_activation_and_credentials() -> None:
    """Operators retain target-allowlist, sandbox, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/noema#404",
        "ContextualWisdomLab/noema#403",
        "ContextualWisdomLab/noema#71",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    quality = (
        ...
        "  pull_request:\n"
        "    paths:\n"
        "      - .github/workflows/noema-hourly-review-repair.yml\n"
        "  push:\n"
        "    paths:\n"
        "      - docs/doctoring/noema-hourly-review-caller.md\n"
        "          python -m compileall -q \\\n"
        "            tests/test_noema_hourly_review_caller.py\n"
        "          git diff --check\n"
    )
    ...
    assert pull_request_paths == {".github/workflows/noema-hourly-review-repair.yml"}
    assert push_paths == {"docs/doctoring/noema-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_noema_hourly_review_caller.py"}
    assert "docs/doctoring/noema-hourly-review-caller.md" not in pull_request_paths
    assert ".github/workflows/noema-hourly-review-repair.yml" not in compileall_paths


def test_focused_quality_workflow_tracks_noema_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/noema-hourly-review-repair.yml"
    doctoring = "docs/doctoring/noema-hourly-review-caller.md"
    contract = "tests/test_noema_hourly_review_caller.py"
