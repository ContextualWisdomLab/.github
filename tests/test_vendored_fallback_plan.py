"""Complete branch tests for the vendored fallback planner and value objects."""
...
from contextual_orchestrator._fallback_plan import build_fallback_plan  # noqa: E402
...
def candidate(
    ...
    *,
    ...
    """Build a concise trusted candidate for policy tests."""
...
def test_context_and_collection_types_fail_closed() -> None:
    ...
    with pytest.raises(CandidateValidationError, match="non-empty"):
        candidate("candidate", "model/one", visibilities=frozenset())
    with pytest.raises(CandidateValidationError, match="non-empty"):
        candidate(
