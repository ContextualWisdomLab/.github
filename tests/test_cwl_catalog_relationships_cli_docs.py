...
PRODUCTION = tuple((ROOT / "scripts/ci").glob("cwl_catalog_*.py")) + (ROOT / "scripts/ci/validate_cwl_ecosystem_catalog.py",)
...
def invalid_relation(tmp_path: Path, field: str, value: object, message: str) -> None:
    ...
    catalog = load_catalog(); first_relation(catalog)[field] = value
    with pytest.raises(CatalogValidationError, match=message):
        validator.validate_catalog(write_catalog_tree(tmp_path, catalog))


def test_relationship_fields_security_and_unknown_services_are_strict(tmp_path: Path) -> None:
    ...
    cases = (
        ...
        ("authoritative_data_owner_service_id", "unknown_service", "unknown authoritative"),
...
def test_relationship_graph_data_flow_authority_and_maturity_invariants(tmp_path: Path) -> None:
    ...
    mutations = [
        ...
        (lambda r: r.__setitem__("consumer_service_id", r["provider_service_id"]), "self-edge"),
        (lambda r: (r.__setitem__("evidence_class", "inferred_relationship"), r.__setitem__("may_update_authoritative_fact", True)), "inferred relationship"),
        (lambda r: (r.__setitem__("data_flow_class", "no_business_data"), r.__setitem__("data_classification", "restricted_identity")), "no_business_data"),
        (lambda r: (r.__setitem__("contract_kind", "build_control"), r.__setitem__("data_flow_class", "purpose_bound_projection")), "build_control"),
        (lambda r: (r.__setitem__("data_classification", "restricted_identity"), r.__setitem__("data_flow_class", "schema_contract")), "restricted data"),
        (lambda r: (r.__setitem__("may_update_authoritative_fact", True), r.__setitem__("authoritative_data_owner_service_id", "identity_federation")), "authoritative consumer"),
        (lambda r: r.__setitem__("maturity", "released"), "exceeds endpoint maturity"),
    ]
    for index, (mutate, message) in enumerate(mutations):
        catalog = load_catalog(); mutate(first_relation(catalog))
        with pytest.raises(CatalogValidationError, match=message):
            validator.validate_catalog(write_catalog_tree(tmp_path / str(index), catalog))
    catalog = load_catalog(); catalog["relationships"][1]["relationship_id"] = catalog["relationships"][0]["relationship_id"]
    ...
    catalog = load_catalog(); catalog["relationships"][0] = "bad"
    with pytest.raises(CatalogValidationError, match=r"relationships\[0\] must be an object"):
        validator.validate_catalog(write_catalog_tree(tmp_path / "shape", catalog))


def test_authoritative_update_is_accepted_only_at_authoritative_consumer(tmp_path: Path) -> None:
    """An explicitly authorized command may update only the authoritative consumer."""

    catalog = load_catalog(); relation = first_relation(catalog)
...
def test_documentation_workflow_and_docstrings_are_complete() -> None:
    ...
    for token in ("authoritative data owner", "direct cross-repository sql", "purpose-bound", "raw pii", "csap", "soc 2", "customer next action", "rollback"):
        assert token in text
    index = INDEX.read_text(encoding="utf-8")
    assert "CWL_ECOSYSTEM_INTEGRATION_CONTRACT.md" in index
    assert "CWL_REPOSITORY_RESPONSIBILITY_CATALOG.md" in index
    assert "repository responsibility catalogue" in CHANGELOG.read_text(encoding="utf-8").lower()
    adr = ADR.read_text(encoding="utf-8")
    assert "## Decision" in adr and "## Consequences" in adr
    doctoring = DOCTORING.read_text(encoding="utf-8")
    for token in ("JSON Schema Draft 2020-12", "OpenAPI 3.2.0", "AsyncAPI 3.1.0", "CloudEvents 1.0", "W3C PROV-O"):
        assert token in doctoring
    workflow = WORKFLOW.read_text(encoding="utf-8")
    for token in ("contents: read", "persist-credentials: false", "coverage run --branch", "--fail-under=100", "github.event.pull_request.head.sha"):
        assert token in workflow
    for path in PRODUCTION:
        tree = ast.parse(path.read_text(encoding="utf-8"))
        assert ast.get_docstring(tree)
        assert [node.name for node in ast.walk(tree) if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)) and not ast.get_docstring(node)] == []


def test_cli_and_callable_entry_points_use_stable_codes(tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    accepted = subprocess.run([sys.executable, str(script), str(CATALOG)], cwd=ROOT, check=False, capture_output=True, text=True)
    assert accepted.returncode == 0 and "validated" in accepted.stdout and accepted.stderr == ""
    catalog = load_catalog(); first_relation(catalog)["direct_cross_repository_sql"] = True
    rejected_path = write_catalog_tree(tmp_path / "rejected", catalog)
    rejected = subprocess.run([sys.executable, str(script), str(rejected_path)], cwd=ROOT, check=False, capture_output=True, text=True)
    assert rejected.returncode == 2 and rejected.stdout == "" and "validation failed" in rejected.stderr and len(rejected.stderr) < 1024
