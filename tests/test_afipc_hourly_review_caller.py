"""Contract tests for aFIPC's bounded hourly review-repair caller."""

from pathlib import Path


CALLER = Path(".github/workflows/afipc-hourly-review-repair.yml")
DOCTORING = Path("docs/doctoring/afipc-hourly-review-caller.md")
...
def test_afipc_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """aFIPC receives one realistic calibration repair without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "2 * * * *"' in caller
    assert "group: afipc-hourly-review-repair" in caller
    assert "cancel-in-progress: false" in caller
    assert "uses: ./.github/workflows/pr-review-fix-scheduler.yml" in caller
    assert "target_repository: ContextualWisdomLab/aFIPC" in caller
...
def test_afipc_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_afipc_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/aFIPC" not in _read(SCHEDULER)


def test_afipc_doctoring_records_fipc_activation_and_credentials() -> None:
    """Operators retain target-allowlist, FIPC, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/aFIPC#259",
        "ContextualWisdomLab/aFIPC#263",
        "ContextualWisdomLab/aFIPC#261",
        "ContextualWisdomLab/aFIPC#262",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    assert pull_request_paths == {".github/workflows/afipc-hourly-review-repair.yml"}
    assert push_paths == {"docs/doctoring/afipc-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_afipc_hourly_review_caller.py"}
    assert "docs/doctoring/afipc-hourly-review-caller.md" not in pull_request_paths
    assert ".github/workflows/afipc-hourly-review-repair.yml" not in compileall_paths


def test_focused_quality_workflow_tracks_afipc_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/afipc-hourly-review-repair.yml"
    doctoring = "docs/doctoring/afipc-hourly-review-caller.md"
    contract = "tests/test_afipc_hourly_review_caller.py"
