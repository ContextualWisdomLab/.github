...
def test_writer_uses_contextual_gateway_with_client_owned_tool_loop() -> None:
    """Pin gateway model selection and the explicit OpenCode tool-loop contract."""
    ...
    assert '"X-Contextual-Orchestrator-Tool-Loop": "v1"' in workflow
    assert '"apiKey": "{env:CONTEXTUAL_ORCHESTRATOR_TOKEN}"' in workflow
