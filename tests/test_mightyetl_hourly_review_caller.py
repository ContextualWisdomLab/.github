"""Contract tests for mightyETL's bounded hourly review-repair caller."""
...
def test_mightyetl_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """mightyETL receives one realistic CDC repair without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "8 * * * *"' in caller
    assert "group: mightyetl-hourly-review-repair" in caller
    assert "cancel-in-progress: false" in caller
    assert "uses: ./.github/workflows/pr-review-fix-scheduler.yml" in caller
    assert "target_repository: ContextualWisdomLab/mightyETL" in caller
...
def test_mightyetl_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_mightyetl_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/mightyETL" not in _read(SCHEDULER)


def test_mightyetl_doctoring_records_cdc_activation_and_credentials() -> None:
    """Operators retain target-allowlist, CDC, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/mightyETL#327",
        "ContextualWisdomLab/mightyETL#326",
        "ContextualWisdomLab/mightyETL#322",
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    assert pull_request_paths == {".github/workflows/mightyetl-hourly-review-repair.yml"}
    assert push_paths == {"docs/doctoring/mightyetl-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_mightyetl_hourly_review_caller.py"}
    assert "docs/doctoring/mightyetl-hourly-review-caller.md" not in pull_request_paths
    assert ".github/workflows/mightyetl-hourly-review-repair.yml" not in compileall_paths


def test_focused_quality_workflow_tracks_mightyetl_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/mightyetl-hourly-review-repair.yml"
    doctoring = "docs/doctoring/mightyetl-hourly-review-caller.md"
    contract = "tests/test_mightyetl_hourly_review_caller.py"
