"""Contract tests for macos_utility_packs' bounded hourly review-repair caller."""

from pathlib import Path


CALLER = Path(".github/workflows/macos_utility_packs-hourly-review-repair.yml")
DOCTORING = Path("docs/doctoring/macos-utility-packs-hourly-review-caller.md")
...
def test_macos_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """macos_utility_packs receives one realistic bootstrap repair without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "44 * * * *"' in caller
    assert "group: macos_utility_packs-hourly-review-repair" in caller
    assert "cancel-in-progress: false" in caller
    assert "uses: ./.github/workflows/pr-review-fix-scheduler.yml" in caller
    assert "target_repository: ContextualWisdomLab/macos_utility_packs" in caller
...
def test_macos_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_macos_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/macos_utility_packs" not in _read(SCHEDULER)


def test_macos_doctoring_records_activation_and_credentials() -> None:
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/macos_utility_packs#3",
        "ContextualWisdomLab/macos_utility_packs#2",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    assert push_paths == {"docs/doctoring/macos-utility-packs-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_macos_utility_packs_hourly_review_caller.py"}
    assert (
        "docs/doctoring/macos-utility-packs-hourly-review-caller.md"
        not in pull_request_paths
    )
    assert (
        ".github/workflows/macos_utility_packs-hourly-review-repair.yml"
...
def test_focused_quality_workflow_tracks_macos_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/macos_utility_packs-hourly-review-repair.yml"
    doctoring = "docs/doctoring/macos-utility-packs-hourly-review-caller.md"
    contract = "tests/test_macos_utility_packs_hourly_review_caller.py"
