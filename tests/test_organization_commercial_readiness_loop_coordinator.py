...
def test_parallel_inspection_errors_are_sorted_by_repository(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Completion order must not leak into the serialized inspection report."""
    client = FakeClient(
        [repository_payload("alpha"), repository_payload("beta")],
        {
            "ContextualWisdomLab/alpha": [GitHubError("alpha failed")],
            "ContextualWisdomLab/beta": [GitHubError("beta failed")],
        },
    )
    monkeypatch.setattr(
        "scripts.ci.organization_commercial_readiness_loop.concurrent.futures.as_completed",
...
def test_parallel_snapshot_exception_waits_for_running_work(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """An unexpected result failure must drain running work before returning."""
    shutdown_calls: list[tuple[bool, bool]] = []

    class FailedFuture:
        """Raise an unexpected snapshot result error."""

        def result(self) -> None:
            """Model an invalid upstream response escaping snapshot()."""
            raise ValueError("invalid snapshot payload")

    class RecordingExecutor:
        """Record cleanup semantics without starting worker threads."""

        def __init__(self, *, max_workers: int) -> None:
            assert max_workers == 2

        def submit(self, function: object, repository: object) -> FailedFuture:
            del function, repository
            return FailedFuture()
    ...
    monkeypatch.setattr(
        "scripts.ci.organization_commercial_readiness_loop.concurrent.futures.ThreadPoolExecutor",
        RecordingExecutor,
    )
    monkeypatch.setattr(
        "scripts.ci.organization_commercial_readiness_loop.concurrent.futures.as_completed",
    ...
    with pytest.raises(ValueError, match="invalid snapshot payload"):
        run_once(client, organization="ContextualWisdomLab", rotation_seed=0)
