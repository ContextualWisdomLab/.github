...
def test_schemas_and_positive_catalogue_align_with_production_constants() -> None:
    ...
    assert set(service_schema["properties"]["integration_mode"]["enum"]) == set(
        INTEGRATION_MODES
    )
    assert set(
        catalog_schema["$defs"]["relationship"]["properties"]["data_flow_class"]["enum"]
    ) == set(DATA_FLOW_CLASSES)
    validator.validate_catalog(CATALOG)


def test_strict_json_rejects_missing_symlink_oversized_encoding_syntax_duplicates_and_nonfinite(
...
def test_bounded_shape_rejects_depth_cardinality_and_string_limits(
...
def test_manifest_resolution_is_rooted_and_symlink_safe(tmp_path: Path) -> None:
    ...
    service_dir.mkdir()
    manifest = service_dir / "valid_service.json"
    ...
    internal_target = tmp_path / "target"
    internal_target.mkdir()
    (internal_target / "inside.json").write_text("{}", encoding="utf-8")
    internal_link = tmp_path / "inside_link"
    internal_link.symlink_to(internal_target, target_is_directory=True)
    with pytest.raises(CatalogValidationError, match="symbolic link"):
...
def test_primitive_contract_helpers_cover_success_and_failure_paths() -> None:
    ...
    assert require_repository("ContextualWisdomLab/.github", "repo").endswith(
        "/.github"
    )
    ...
    assert require_immutable_reference("contract:cwl/example@1.0.0", "ref").endswith(
        "@1.0.0"
    )
...
def test_catalogue_root_rejects_shape_identity_and_manifest_reference_errors(
    tmp_path: Path,
) -> None:
    ...
    base = load_catalog()
    base["extra"] = True
    cases.append((base, "unknown properties"))
    base = load_catalog()
    base.pop("catalog_id")
    cases.append((base, "missing properties"))
    base = load_catalog()
    base["schema_version"] = "2.0.0"
    cases.append((base, "schema_version"))
    base = load_catalog()
    base["catalog_id"] = "single"
    cases.append((base, "catalog_id"))
    base = load_catalog()
    base["catalog_version"] = "v1"
    cases.append((base, "catalog_version"))
    base = load_catalog()
    base["maturity"] = "complete"
    cases.append((base, "controlled"))
    base = load_catalog()
    base["service_manifests"] = []
    cases.append((base, "at least one"))
    base = load_catalog()
    base["service_manifests"] = "bad"
    cases.append((base, "must be an array"))
    base = load_catalog()
    base["relationships"] = "bad"
    cases.append((base, "must be an array"))
