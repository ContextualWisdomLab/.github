"""Contract tests for html4tree's bounded hourly review-repair caller."""

from pathlib import Path


CALLER = Path(".github/workflows/html4tree-hourly-review-repair.yml")
DOCTORING = Path("docs/doctoring/html4tree-hourly-review-caller.md")
...
def test_html4tree_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """html4tree receives one realistic directory-index repair without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "15 * * * *"' in caller
    assert "group: html4tree-hourly-review-repair" in caller
    assert "cancel-in-progress: false" in caller
    assert "uses: ./.github/workflows/pr-review-fix-scheduler.yml" in caller
    assert "target_repository: ContextualWisdomLab/html4tree" in caller
...
def test_html4tree_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_html4tree_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/html4tree" not in _read(SCHEDULER)


def test_html4tree_doctoring_records_fork_activation_and_credentials() -> None:
    """Operators retain target-allowlist, fork, and approval prerequisites."""
    doctoring = _read(DOCTORING)

    for phrase in (
        "ContextualWisdomLab/html4tree",
        ...
        "APA 7th references",
        "ContextualWisdomLab/html4tree#475",
        "ContextualWisdomLab/html4tree#472",
        "ContextualWisdomLab/html4tree#454",
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    assert push_paths == {"docs/doctoring/html4tree-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_html4tree_hourly_review_caller.py"}
    assert "docs/doctoring/html4tree-hourly-review-caller.md" not in pull_request_paths
    assert (
        ".github/workflows/html4tree-hourly-review-repair.yml"
...
def test_focused_quality_workflow_tracks_html4tree_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/html4tree-hourly-review-repair.yml"
    doctoring = "docs/doctoring/html4tree-hourly-review-caller.md"
    contract = "tests/test_html4tree_hourly_review_caller.py"
