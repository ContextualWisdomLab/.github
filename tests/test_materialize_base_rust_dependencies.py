...
def test_multiple_workspace_roots_fail_closed(tmp_path: Path) -> None:
    """An ambiguous multi-root layout refuses to guess which lock is authoritative."""
    repo = tmp_path / "repo"
    _init_repo(repo)
    for name in ("a", "b"):
        crate_dir = repo / name
    ...
    with pytest.raises(RuntimeError, match="more than one Cargo workspace root"):
...
def test_select_vendor_root_workspace_without_sibling_lock_raises(
    ...
    with pytest.raises(RuntimeError, match="no sibling Cargo.lock"):
        materializer._select_vendor_root(Path("/unused"), "a" * 40, ["Cargo.toml"])
...
def test_select_vendor_root_returns_single_standalone_crate(
    ...
    root = materializer._select_vendor_root(
...
def test_select_vendor_root_single_lock_without_manifest_raises(
    ...
    with pytest.raises(RuntimeError, match="no sibling Cargo.toml"):
        materializer._select_vendor_root(Path("/unused"), "a" * 40, ["crate-a/Cargo.lock"])


def test_select_vendor_root_multiple_locks_without_workspace_raises(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Two independent standalone crates with no shared workspace root fail closed."""
    monkeypatch.setattr(materializer, "_git", lambda *_a, **_k: b'[package]\nname = "x"\n')
    with pytest.raises(RuntimeError, match="more than one Cargo.lock"):
        materializer._select_vendor_root(
            Path("/unused"),
            "a" * 40,
