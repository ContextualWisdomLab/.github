"""Contract tests for Keyverse's bounded hourly review-repair caller."""

from pathlib import Path


CALLER = Path(".github/workflows/keyverse-hourly-review-repair.yml")
DOCTORING = Path("docs/doctoring/keyverse-hourly-review-caller.md")
...
def test_keyverse_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """Keyverse receives one realistic IdP repair opportunity without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "29 * * * *"' in caller
    assert "group: keyverse-hourly-review-repair" in caller
    assert "cancel-in-progress: false" in caller
    assert "uses: ./.github/workflows/pr-review-fix-scheduler.yml" in caller
    assert "target_repository: ContextualWisdomLab/keyverse" in caller
...
def test_keyverse_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_keyverse_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/keyverse" not in _read(SCHEDULER)


def test_keyverse_doctoring_records_idp_activation_and_credentials() -> None:
    """Operators retain target-allowlist, IdP, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/keyverse#83",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    assert pull_request_paths == {".github/workflows/keyverse-hourly-review-repair.yml"}
    assert push_paths == {"docs/doctoring/keyverse-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_keyverse_hourly_review_caller.py"}
    assert "docs/doctoring/keyverse-hourly-review-caller.md" not in pull_request_paths
    assert ".github/workflows/keyverse-hourly-review-repair.yml" not in compileall_paths


def test_focused_quality_workflow_tracks_keyverse_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/keyverse-hourly-review-repair.yml"
    doctoring = "docs/doctoring/keyverse-hourly-review-caller.md"
    contract = "tests/test_keyverse_hourly_review_caller.py"
