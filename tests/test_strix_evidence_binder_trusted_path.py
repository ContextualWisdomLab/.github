...
def test_binder_runs_when_the_scanned_repository_has_no_central_scripts(tmp_path: Path) -> None:
    """A consumer repo need not contain the central Strix binder.

    ``strix.yml`` runs the trusted ``.github`` gate with ``STRIX_REPO_ROOT`` set
    to the consumer checkout. The evidence binder must therefore resolve next
    to the trusted gate rather than under the scanned repository root.
    """
