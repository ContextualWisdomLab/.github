"""Contract tests for semantic-data-portal's bounded hourly review-repair caller."""
...
def test_semantic_data_portal_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """semantic-data-portal receives one catalog-plane repair without cancellation."""
    caller = _read(CALLER)
...
def test_semantic_data_portal_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_semantic_data_portal_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/semantic-data-portal" not in _read(SCHEDULER)


def test_semantic_data_portal_doctoring_records_catalog_activation_and_credentials() -> None:
    """Operators retain target-allowlist, catalog plane, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/semantic-data-portal#61",
        "ContextualWisdomLab/semantic-data-portal#59",
        "ContextualWisdomLab/semantic-data-portal#58",
        "ContextualWisdomLab/semantic-data-portal#35",
    ):
...
def test_focused_quality_workflow_tracks_semantic_data_portal_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/semantic-data-portal-hourly-review-repair.yml"
    doctoring = "docs/doctoring/semantic-data-portal-hourly-review-caller.md"
    contract = "tests/test_semantic_data_portal_hourly_review_caller.py"
