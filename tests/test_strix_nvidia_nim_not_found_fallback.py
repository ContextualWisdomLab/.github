...
def _run_strix_once_resolves_child_model(model: str, primary_model: str) -> str:
    ...
    Extracts the exact statements `run_strix_once` uses to compute the model
    it hands to LiteLLM, rather than reimplementing that composition, so a
    future call-site edit that stops normalizing the workflow-facing alias
    before dispatch fails this test instead of only failing in CI against
    live NVIDIA NIM traffic.
    """
...
class StrixNvidiaNotFoundFallbackTests(unittest.TestCase):
    ...
    def test_run_strix_once_normalizes_hyphenated_fallback_before_dispatch(
        ...
        `normalize_model()` rewrites the workflow-facing `openai-direct/` alias
        to LiteLLM's `openai_direct/` selector, and `child_model_for_api_base()`
        rewrites `openai_direct/*` to `openai/*`. Observed live: NVIDIA NIM
        returned 404, the run advanced to `openai-direct/gpt-5.6-luna`, and
        LiteLLM recorded `model=openai-direct/gpt-5.6-luna` before
        `BadRequestError: LLM Provider NOT provided` ended the scan.
        """
