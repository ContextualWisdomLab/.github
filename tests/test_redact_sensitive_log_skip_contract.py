...
def _base_style_redact_assignments(text: str) -> str:
    """Reproduce the former one-character failure-path advancement."""
    output: list[str] = []
    cursor = 0
...
def test_skip_index_matches_base_failure_path_on_adversarial_inputs(source: str) -> None:
    """Skipping rejected runs must preserve the former parser's redaction output."""
...
def test_sensitive_key_search_is_unanchored_for_shifted_key_runs() -> None:
    """A rejected full key cannot contain a suffix that newly matches the pattern."""
...
def test_shifted_and_malformed_sensitive_keys_remain_redacted() -> None:
    """Shifted, digit-prefixed, and malformed quoted keys never expose values."""
