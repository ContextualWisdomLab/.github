"""Contract tests for psychometrics-commons' bounded hourly review-repair caller."""
...
def test_psychometrics_commons_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """psychometrics-commons receives one realistic repair without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "9 * * * *"' in caller
    assert "group: psychometrics-commons-hourly-review-repair" in caller
    assert "cancel-in-progress: false" in caller
    assert "uses: ./.github/workflows/pr-review-fix-scheduler.yml" in caller
    assert "target_repository: ContextualWisdomLab/psychometrics-commons" in caller
...
def test_psychometrics_commons_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_psychometrics_commons_doctoring_records_measurement_activation() -> None:
    """Operators retain target-allowlist, measurement, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/psychometrics-commons#244",
        "ContextualWisdomLab/psychometrics-commons#242",
        "ContextualWisdomLab/psychometrics-commons#240",
        "ContextualWisdomLab/psychometrics-commons#237",
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    assert push_paths == {"docs/doctoring/psychometrics-commons-hourly-review-caller.md"}
