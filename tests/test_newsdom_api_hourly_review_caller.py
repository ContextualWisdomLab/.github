"""Contract tests for newsdom-api's bounded hourly review-repair caller."""

from pathlib import Path


CALLER = Path(".github/workflows/newsdom-api-hourly-review-repair.yml")
DOCTORING = Path("docs/doctoring/newsdom-api-hourly-review-caller.md")
...
def test_newsdom_api_caller_is_hourly_bounded_and_non_cancelling() -> None:
    ...
    assert "target_repository: ContextualWisdomLab/newsdom-api" in caller
...
def test_newsdom_api_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/newsdom-api" not in _read(SCHEDULER)


def test_newsdom_api_doctoring_records_dom_activation_and_credentials() -> None:
    """Operators retain target-allowlist, DOM sidecar, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/newsdom-api#634",
        "ContextualWisdomLab/newsdom-api#632",
        "ContextualWisdomLab/newsdom-api#639",
        "ContextualWisdomLab/newsdom-api#638",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    quality = (
        ...
        "  pull_request:\n"
        "    paths:\n"
        "      - .github/workflows/newsdom-api-hourly-review-repair.yml\n"
        "  push:\n"
        "    paths:\n"
        "      - docs/doctoring/newsdom-api-hourly-review-caller.md\n"
        "          python -m compileall -q \\\n"
        "            tests/test_newsdom_api_hourly_review_caller.py\n"
        "          git diff --check\n"
    )
    ...
    assert pull_request_paths == {".github/workflows/newsdom-api-hourly-review-repair.yml"}
    assert push_paths == {"docs/doctoring/newsdom-api-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_newsdom_api_hourly_review_caller.py"}
    assert "docs/doctoring/newsdom-api-hourly-review-caller.md" not in pull_request_paths
    assert ".github/workflows/newsdom-api-hourly-review-repair.yml" not in compileall_paths


def test_focused_quality_workflow_tracks_newsdom_api_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/newsdom-api-hourly-review-repair.yml"
    doctoring = "docs/doctoring/newsdom-api-hourly-review-caller.md"
    contract = "tests/test_newsdom_api_hourly_review_caller.py"
