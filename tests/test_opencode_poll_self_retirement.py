...
def _run_poll_loop(
    ...
    *,
    ...
    ``date_epochs``, when given, stubs ``date +%s`` to return each listed
    epoch in turn (clamped to the last entry once exhausted) instead of the
    real clock -- letting a test fast-forward past the real
    ``max_poll_wall_clock_seconds`` deadline after a chosen number of
...
""",
    ...
    script = "\n".join(
        (
            ...
            'max_poll_wall_clock_seconds=10800',
            'poll_deadline_epoch=$(( $(date +%s) + max_poll_wall_clock_seconds ))',
            "while :; do",
            _poll_loop(),
            "done",
        )
...
def test_poll_fails_closed_after_wall_clock_deadline_with_every_gh_call_succeeding(
    ...
    (`max_poll_wall_clock_seconds` / `poll_deadline_epoch`) can release the
    ...
    assert result.returncode == 1
    assert (
        "::error::No verdict after 180 minutes of polling; failing closed "
        "and releasing the runner." in result.stdout
    )
...
def test_poll_wall_clock_deadline_does_not_interfere_with_a_fast_verdict(
    ...
    assert "No verdict after" not in result.stdout
...
def test_wall_clock_deadline_is_distinct_from_and_additional_to_transport_counter() -> None:
    ...
    `max_poll_transport_failures` (existing) and `max_poll_wall_clock_seconds`
    / `poll_deadline_epoch` (new) must be present, and the wall-clock check
    must live inside the `while :; do` loop body -- not as a job-level
    `timeout-minutes:`, which would kill the runner mid-request instead of
    failing closed with a clear diagnostic.
    """
    target_job = WORKFLOW.read_text(encoding="utf-8").split(
        "  opencode-review-target:\n", 1
    )[1].split("\n  cancel-superseded-opencode-review-runs:\n", 1)[0]
    assert "max_poll_transport_failures=3" in target_job
    assert "max_poll_wall_clock_seconds=10800" in target_job
    assert (
        "poll_deadline_epoch=$(( $(date +%s) + max_poll_wall_clock_seconds ))"
        in target_job
    )
    loop = _poll_loop()
    assert (
        'if [ "$(date +%s)" -ge "$poll_deadline_epoch" ]; then' in loop
    )
    assert (
        '::error::No verdict after $(( max_poll_wall_clock_seconds / 60 )) '
        "minutes of polling; failing closed and releasing the runner." in loop
    )
