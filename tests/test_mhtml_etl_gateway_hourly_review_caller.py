"""Contract tests for mhtml-etl-gateway's bounded hourly review-repair caller."""

from pathlib import Path


CALLER = Path(".github/workflows/mhtml-etl-gateway-hourly-review-repair.yml")
DOCTORING = Path("docs/doctoring/mhtml-etl-gateway-hourly-review-caller.md")
...
def test_mhtml_etl_gateway_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """mhtml-etl-gateway receives one realistic ingest repair without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "15 * * * *"' in caller
    assert "governance-risk-compliance (43)" in caller
    assert "newsdom-api (18)" in caller
    ...
    assert "target_repository: ContextualWisdomLab/mhtml-etl-gateway" in caller
...
def test_mhtml_etl_gateway_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_mhtml_etl_gateway_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/mhtml-etl-gateway" not in _read(SCHEDULER)


def test_mhtml_etl_gateway_doctoring_records_ingest_activation_and_credentials() -> None:
    """Operators retain target-allowlist, ingest, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/mhtml-etl-gateway#34",
        "ContextualWisdomLab/mhtml-etl-gateway#36",
        "ContextualWisdomLab/mhtml-etl-gateway#35",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    quality = (
        ...
        "  pull_request:\n"
        "    paths:\n"
        "      - .github/workflows/mhtml-etl-gateway-hourly-review-repair.yml\n"
        "  push:\n"
        "    paths:\n"
        "      - docs/doctoring/mhtml-etl-gateway-hourly-review-caller.md\n"
        "          python -m compileall -q \\\n"
        "            tests/test_mhtml_etl_gateway_hourly_review_caller.py\n"
        "          git diff --check\n"
    )
    ...
    assert push_paths == {"docs/doctoring/mhtml-etl-gateway-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_mhtml_etl_gateway_hourly_review_caller.py"}
    assert "docs/doctoring/mhtml-etl-gateway-hourly-review-caller.md" not in pull_request_paths
    assert (
        ".github/workflows/mhtml-etl-gateway-hourly-review-repair.yml"
        not in compileall_paths
    )


def test_focused_quality_workflow_tracks_mhtml_etl_gateway_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/mhtml-etl-gateway-hourly-review-repair.yml"
    doctoring = "docs/doctoring/mhtml-etl-gateway-hourly-review-caller.md"
    contract = "tests/test_mhtml_etl_gateway_hourly_review_caller.py"
