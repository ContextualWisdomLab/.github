...
def test_bounded_reader_rejects_descriptor_metadata_change(
    tmp_path: Path, monkeypatch
) -> None:
    """Reject evidence whose descriptor metadata changes during the read."""
    ...
    assert gate._read_bounded_regular(payload, 16) is None


def test_bounded_reader_rejects_live_path_replacement(
    tmp_path: Path, monkeypatch
) -> None:
    """Reject evidence when the lexical path is replaced before final sealing."""
