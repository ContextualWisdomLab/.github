"""Contract tests for LifeOS's bounded hourly review-repair caller."""

from pathlib import Path


CALLER = Path(".github/workflows/life-os-hourly-review-repair.yml")
DOCTORING = Path("docs/doctoring/life-os-hourly-review-caller.md")
...
def test_life_os_caller_is_hourly_bounded_and_non_cancelling() -> None:
    """LifeOS receives one realistic goals or plugin-origin repair without cancellation."""
    caller = _read(CALLER)

    assert 'cron: "6 * * * *"' in caller
    assert "group: life-os-hourly-review-repair" in caller
    assert "cancel-in-progress: false" in caller
    assert "uses: ./.github/workflows/pr-review-fix-scheduler.yml" in caller
    assert "target_repository: ContextualWisdomLab/life-os" in caller
...
def test_life_os_caller_preserves_oidc_and_explicit_secret_scope() -> None:
...
def test_life_os_target_is_not_hard_coded_in_shared_scheduler() -> None:
    """Product identity remains in the thin caller rather than the engine."""
    assert "ContextualWisdomLab/life-os" not in _read(SCHEDULER)


def test_life_os_doctoring_records_product_activation_and_credentials() -> None:
    """Operators retain target-allowlist, LifeOS, and approval prerequisites."""
    ...
    for phrase in (
        ...
        "APA 7th references",
        "ContextualWisdomLab/life-os#206",
        "ContextualWisdomLab/life-os#205",
        "ContextualWisdomLab/life-os#199",
        "ContextualWisdomLab/life-os#198",
        "ContextualWisdomLab/life-os#145",
    ):
...
def test_path_block_helpers_keep_trigger_and_compileall_sets_disjoint() -> None:
    ...
    quality = (
        ...
        "  pull_request:\n"
        "    paths:\n"
        "      - .github/workflows/life-os-hourly-review-repair.yml\n"
        "  push:\n"
        "    paths:\n"
        "      - docs/doctoring/life-os-hourly-review-caller.md\n"
        "          python -m compileall -q \\\n"
        "            tests/test_life_os_hourly_review_caller.py\n"
        "          git diff --check\n"
    )
    ...
    assert pull_request_paths == {".github/workflows/life-os-hourly-review-repair.yml"}
    assert push_paths == {"docs/doctoring/life-os-hourly-review-caller.md"}
    assert compileall_paths == {"tests/test_life_os_hourly_review_caller.py"}
    assert "docs/doctoring/life-os-hourly-review-caller.md" not in pull_request_paths
    assert ".github/workflows/life-os-hourly-review-repair.yml" not in compileall_paths


def test_focused_quality_workflow_tracks_life_os_contracts() -> None:
    """Caller, test, and doctoring edits always rerun the focused gate."""
    quality = _read(QUALITY_WORKFLOW)
    pull_request_paths = _yaml_path_entries(_trigger_path_block(quality, "pull_request"))
    push_paths = _yaml_path_entries(_trigger_path_block(quality, "push"))
    compileall_paths = _yaml_path_entries(_compileall_block(quality))
    caller = ".github/workflows/life-os-hourly-review-repair.yml"
    doctoring = "docs/doctoring/life-os-hourly-review-caller.md"
    contract = "tests/test_life_os_hourly_review_caller.py"
