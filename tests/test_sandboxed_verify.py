...
def test_scrubbed_env_allows_named_credentials_without_printing_values(monkeypatch, tmp_path, capsys):
    ...
    assert all("731" not in argument for argument in payload["command"])
...
def test_main_rejects_short_allowed_values_without_changing_result_schema(
    ...
    assert payload["cwd"].endswith("repo")
    assert sandboxed_verify.REDACTION_MARKER not in payload["cwd"]
    assert sandboxed_verify.REDACTION_MARKER not in payload["sandbox"]
