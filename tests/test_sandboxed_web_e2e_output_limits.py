...
def test_start_service_enforces_real_log_file_ceiling(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert service.log_path.stat().st_size <= 4097
...
def test_normal_services_and_e2e_preserve_existing_success_contract(
    ...
    exit_code = sandboxed_web_e2e.main(
        [
            ...
            str(_repository(tmp_path)),
            "--backend-cmd",
            _command("import time; print('backend-ready', flush=True); time.sleep(30)"),
            "--frontend-cmd",
            _command("import time; print('frontend-ready', flush=True); time.sleep(30)"),
            "--e2e-cmd",
...
def test_cli_rejects_unsafe_command_and_service_budgets(tmp_path: Path) -> None:
...
def test_kept_sandbox_service_file_never_exceeds_kernel_ceiling(
    ...
    try:
        assert exit_code == bounded.OUTPUT_LIMIT_EXIT_CODE
        assert (sandbox_path / "logs" / "backend.log").stat().st_size <= 4097
    finally:
